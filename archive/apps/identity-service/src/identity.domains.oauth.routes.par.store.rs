...
pub(crate) async fn consume_pushed_parameters(
    redis: &nvbes_redis::RedisPool,
    request_uri: &str,
    client_id: &str,
) -> Result<serde_json::Map<String, serde_json::Value>, AppError> {
    let lock_key = format!("oauth-par-consume:{request_uri}");
    let locked = nvbes_redis::lock::acquire(redis, &lock_key, 15)
        .await
        .map_err(|error| {
            AppError::internal(
                "pushed_authorization_request_lock_failed",
                error.to_string(),
            )
        })?;
    if !locked {
        return Err(AppError::bad_request(
            "invalid_request_uri",
            "The request_uri is already being used.",
        ));
    }

    let result = async {
        let parameters = resolve_pushed_parameters(redis, request_uri, client_id).await?;
        mark_par_used(redis, request_uri).await?;
        Ok(parameters)
    }
    .await;
