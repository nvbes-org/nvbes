...
fn validate_pkce_for_authorize_rejects_plain_method() {
    let error = validate_pkce_for_authorize(
        "public",
...
#[test]
fn validate_pkce_for_authorize_rejects_malformed_s256_challenge() {
    let error = validate_pkce_for_authorize("public", Some("too-short"), Some("S256"))
        .expect_err("malformed challenge should fail");

    assert_eq!(error.code, "invalid_code_challenge");
}

#[test]
fn validate_pkce_for_exchange_rejects_malformed_verifier() {
    let error = validate_pkce_for_exchange(
        "public",
        Some("E9Melhoa2OwvFrEMTJguCHaoeK1t8URWbuGJSstw-cM"),
        Some("S256"),
        Some("too-short"),
    )
    .expect_err("malformed verifier should fail");
