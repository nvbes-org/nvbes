...
pub async fn run_once(state: &AppState) -> anyhow::Result<()> {
    let identity_check_in = start_worker_monitor_check_in(
        &state.config.environment,
        &worker_monitor_slug(
            "identity-worker",
            "housekeeping-expired-unverified-identities",
        ),
        HOUSEKEEPING_MONITOR_SCHEDULE,
    );

    let result =
        nvbes_product_identity::auth::email_verification::cleanup_expired_unverified_identities(
            &state.db,
    ...
    let deleted = match result {
        Ok(deleted) => {
            identity_check_in.finish_ok();
            deleted
        }
        Err(error) => {
            identity_check_in.finish_error();
            return Err(error);
        }
    };

    if deleted > 0 {
        tracing::info!(deleted, "Expired unverified identities cleaned up");
    }
