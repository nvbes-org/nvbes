    ...
    <section className="mx-auto grid max-w-5xl gap-6 px-6 py-10">
      <header>
        <h1 className="text-3xl font-semibold md:text-5xl">
          {openApiQuery.data?.info?.title ?? 'Developer Service reference'}
        </h1>
      ...
      {hasApiError ? (
        <div className="rounded-md border border-destructive/30 bg-destructive/10 p-4">
          ...
          <p className="mt-2 text-sm leading-6 text-muted-foreground">
            OpenAPI metadata could not be loaded from the Developer Service. Start the API service
            or configure the console-web proxy target, then reload this page.
          </p>
        </div>
      ) : null}
