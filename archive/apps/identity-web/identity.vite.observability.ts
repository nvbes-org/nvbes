...
export function resolveFaroUrl(envSources: readonly EnvironmentSource[]): string {
  ...
  const required = firstConfiguredValue(envSources, ['NVBES_REQUIRE_FARO_IDENTITY_WEB']) === 'true';

  if (required && !url) {
    throw new Error(
      'Faro is required for identity-web, but VITE_FARO_URL_IDENTITY_WEB is missing. ' +
