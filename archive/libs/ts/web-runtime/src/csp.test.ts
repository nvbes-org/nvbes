...
describe('buildWebCsp', () => {
  it('enforces Trusted Types and denies framing', () => {
    ...
    expect(csp).toContain("'sha256-MADsBPBvAfzKSGq+N7sBxSyONZL+4BuiYOfl+I7SpAE='");
  ...
  it('rejects malformed nonces', () => {
    expect(() => buildWebCsp({ mode: 'production', nonce: "'unsafe-inline'" })).toThrow(
      'CSP nonce',
    );
  });

  it('exports a preload-ready HSTS policy', () => {
    expect(strictTransportSecurity).toBe('max-age=63072000; includeSubDomains; preload');
  });
});
