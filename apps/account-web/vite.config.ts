...
export default defineConfig(({ mode }) => {
  ...
  const accountWebPort = Number(
    process.env.VITE_ACCOUNT_WEB_PORT ||
      localEnv.VITE_ACCOUNT_WEB_PORT ||
      rootEnv.VITE_ACCOUNT_WEB_PORT ||
      3001,
  );
  if (!Number.isInteger(accountWebPort) || accountWebPort < 1024 || accountWebPort > 65_535) {
    throw new Error('VITE_ACCOUNT_WEB_PORT must be an integer between 1024 and 65535');
  }
