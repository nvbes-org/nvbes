...
test.describe('@staging authenticated Account acceptance', () => {
  test('the synthetic release identity can sign in, access itself and sign out', async ({
    page,
  }) => {
    const email = requiredEnvironment('NVBES_STAGING_ACCOUNT_EMAIL');
    const password = requiredEnvironment('NVBES_STAGING_ACCOUNT_PASSWORD');
    ...
    if (meResponse.status() !== 200) {
      throw new Error('Authenticated Account pre-release identity check failed.');
    }
    const body: unknown = await meResponse.json();
    if (!isExpectedIdentity(body, email)) {
      throw new Error('Authenticated Account pre-release identity does not match the fixture.');
    }
