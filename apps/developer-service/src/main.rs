...
async fn main() -> anyhow::Result<()> {
    ...
    nvbes_developer_service::grpc::service::serve(grpc_addr, state, async {
...
fn developer_grpc_port(default_api_port: u16) -> anyhow::Result<u16> {
    match std::env::var(DEVELOPER_GRPC_PORT_ENV) {
        Ok(port) => port
            .parse::<u16>()
            .map_err(|error| anyhow::anyhow!("{DEVELOPER_GRPC_PORT_ENV} is invalid: {error}")),
        Err(std::env::VarError::NotPresent) => default_api_port
            .checked_add(41)
            .ok_or_else(|| anyhow::anyhow!("Default Developer gRPC port overflowed")),
        Err(error) => Err(anyhow::anyhow!(
            "{DEVELOPER_GRPC_PORT_ENV} could not be read: {error}"
        )),
    }
}
...
#[cfg(test)]
mod tests {
    use super::developer_grpc_port;

    #[test]
    fn developer_grpc_port_defaults_after_primary_api_port() {
