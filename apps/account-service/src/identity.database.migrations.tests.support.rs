...
pub fn migration_admin_url() -> Result<String> {
    let value = std::env::var("ACCOUNT_MIGRATION_TEST_ADMIN_URL")
        .or_else(|_| std::env::var("DATABASE_URL"))
        .or_else(|_| std::env::var("NVBES_DATABASE_URL"))
        .context(
            "ACCOUNT_MIGRATION_TEST_ADMIN_URL, DATABASE_URL, or NVBES_DATABASE_URL is required",
        )?;
...
fn migration_run_id() -> Result<&'static str> {
    ...
    let run_id = RUN_ID.get_or_init(|| {
        std::env::var("ACCOUNT_MIGRATION_TEST_RUN_ID")
