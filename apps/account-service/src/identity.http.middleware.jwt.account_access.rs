...
fn validate_account_access(
    ...
    let CredentialSource::OAuthBearer(credential) = &auth.credential_source else {
        return Ok(None);
    };

    let AccountAccess::OAuthScope(required_scope) = access else {
        return Err(AppError::forbidden(
            "browser_session_required",
            "This account operation requires a first-party browser session.",
        ));
    };
