...
pub async fn list_clients(
    ...
    let rows = sqlx::query(
        ...
        SELECT
          ...
          oauth_clients.security_event_receiver_uri,
          oauth_clients.client_assertion_public_key_jwk IS NOT NULL AS client_assertion_public_key_configured,
          oauth_clients.security_profile::text AS security_profile,
          oauth_clients.request_object_signing_jwks IS NOT NULL AS request_object_signing_keys_configured,
          oauth_clients.sender_constraint::text AS sender_constraint,
          oauth_clients.sender_constraint = 'mtls' AS tls_client_certificate_bound_access_tokens,
...
async fn map_oauth_client_row(tenant_id: Uuid, row: PgRow) -> Result<OAuthClientView, AppError> {
    ...
    Ok(OAuthClientView {
        ...
        request_object_signing_keys_configured: row.get("request_object_signing_keys_configured"),
        sender_constraint: row.get("sender_constraint"),
        tls_client_certificate_bound_access_tokens: row
        ...
        security_event_receiver_uri: row.get("security_event_receiver_uri"),
