...
async fn main() -> anyhow::Result<()> {
    ...
    if args.iter().any(|a| a == "--export-openapi") {
        let doc = nvbes_identity_service::http::openapi::IdentityApiDoc::openapi();
        println!("{}", doc.to_json()?);
        return Ok(());
    }
    ...
    let _error_reporting_guard = init_error_reporting_for_service(&config, "identity-service");
    install_safe_panic_hook();
    init_tracing_for_service(&config, "identity-service");
    let _profiling_guard =
        start_continuous_profiling(&config, "identity-service").map_err(anyhow::Error::msg)?;

    let db = nvbes_core::postgres_runtime::connect_pool(&config).await?;

    sqlx::migrate!("./migrations").run(&db).await?;

    let state = nvbes_identity_service::app::AppState::bootstrap(&config, db).await?;
    nvbes_identity_service::domains::oauth::security_events::start_dispatcher(
        state.db.clone(),
        state.jwt.clone(),
    );
    let app = nvbes_identity_service::app::build_router(state.clone());
    ...
    let grpc_addr: SocketAddr =
        format!("0.0.0.0:{}", identity_grpc_port(config.api_port)?).parse()?;
    ...
    if config.mtls_enabled {
        ...
        let mtls_server = axum_server::Server::bind(mtls_addr)
            .acceptor(
                nvbes_identity_service::http::mtls::PeerCertificateAcceptor::new(mtls_acceptor),
            )
        ...
        let grpc_server = nvbes_identity_service::grpc::service::serve(
            grpc_addr,
            state,
        ...
        tracing::info!(%http_addr, "Starting nvbes Identity Service");
        let http_server = axum::serve(
            http_listener,
            app.into_make_service_with_connect_info::<SocketAddr>(),
        )
...
#[cfg(test)]
mod tests {
    use super::{identity_grpc_port, login_protection_required};

    #[test]
    fn identity_grpc_port_defaults_after_primary_api_port() {
        assert_eq!(identity_grpc_port(4000).unwrap(), 4010);
    }
