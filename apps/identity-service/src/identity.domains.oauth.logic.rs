...
pub async fn verify_client_secret_with_overlap(
    ...
    if nvbes_product_identity::oauth::verify_client_secret(secret, main_hash).is_ok() {
        return Ok(());
    }
...
mod tests {
    ...
    fn test_client_secret_hashing_owasp_params() {
        ...
        let hash = nvbes_product_identity::oauth::hash_client_secret(secret)
            .expect("hashing should succeed");

        assert!(hash.contains("$argon2id$"));
        assert!(hash.contains("m=65536,t=3,p=4"));

        nvbes_product_identity::oauth::verify_client_secret(secret, &hash)
            .expect("verification should run");

        let not_ok = nvbes_product_identity::oauth::verify_client_secret("wrong-secret", &hash);
