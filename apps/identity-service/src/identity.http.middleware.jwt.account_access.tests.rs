...
#[test]
fn account_api_rejects_identity_browser_session() {
    ...
    .expect_err("Account API must reject the Identity browser session");

    assert_eq!(error.status, StatusCode::UNAUTHORIZED);
    assert_eq!(error.code, "account_bearer_token_required");
}

#[test]
fn identity_browser_operation_accepts_browser_session() {
    let auth = auth_context(CredentialSource::BrowserSession, "");
    ...
    assert!(
        ...
        .expect("Identity browser operation should accept its session")
        .is_none()
    );
}
