use super::{connect, connect_lazy};
...
#[tokio::test]
async fn connect_succeeds_when_postgres_is_available() {
    if !postgres_reachable() {
        return;
    }
    let url = std::env::var("DATABASE_URL")
        .unwrap_or_else(|_| "postgres://postgres:postgres@127.0.0.1:5432/nvbes_identity".into());
