...
use nvbes_product_account::{AccountError, error::AccountErrorKind};
...
impl JobExecutionError {
    ...
    pub(super) fn from_account(error: &AccountError) -> Self {
        let class = match error.kind {
            AccountErrorKind::Internal => JobFailureClass::Transient,
            AccountErrorKind::BadRequest
            | AccountErrorKind::Unauthorized
            | AccountErrorKind::Forbidden
            | AccountErrorKind::NotFound
            | AccountErrorKind::Conflict => JobFailureClass::Permanent,
        };
        let summary = match class {
            JobFailureClass::Transient => "Account persistence operation failed",
            JobFailureClass::Permanent => "Account job input or state is invalid",
        };
