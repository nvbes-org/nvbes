...
use super::{
    email_delivery::deliver_email, email_from_address, process_data_export::process_data_export,
...
pub(super) async fn execute_job(
    ...
    match job.job_type.as_str() {
        ...
        JOB_DATA_EXPORT => process_data_export(state, job).await.map(|deduplicated| {
            serde_json::json!({
                "status": "exported",
