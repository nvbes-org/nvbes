...
pub async fn stripe_webhook_handler(
    ...
    .await
    {
        ...
        Err(err) => {
            ...
            let _ = sqlx::query("UPDATE billing_webhook_events SET status = 'failed', error_message = $2 WHERE event_id = $1")
                .bind(&event.id)
                .bind(&error_msg)
                .execute(&state.db)
                .await;
...
async fn process_stripe_event(
    ...
    match event_type {
        ...
        | "invoice.payment_failed" => {
            apply_subscription_event(&state.db, event_id, event_type, created, data).await?;
        }
        _ => {
