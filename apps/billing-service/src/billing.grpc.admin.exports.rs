...
use crate::grpc::pb::nvbes::billing::v1::AdminFinanceExport;

pub async fn build_finance_export(
    db: &sqlx::PgPool,
    tenant_id: Uuid,
    export_type: &str,
) -> Result<AdminFinanceExport, Status> {
    ...
    Ok(AdminFinanceExport {
        export_run_id: export_run_id.to_string(),
        filename,
...
fn parse_finance_export_type(value: &str) -> Result<FinanceExportType, Status> {
    match value {
        ...
        _ => Err(Status::invalid_argument(
            "invalid_billing_export_type: Billing export type must be invoices, payments, tax, ledger, customers, or subscriptions.",
        )),
    }
}
...
) -> Result<(Vec<&'static str>, Vec<FinanceExportRow>), Status> {
