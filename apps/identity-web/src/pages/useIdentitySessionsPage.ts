import { identityClient, type AccountSession } from '@nvbes/identity-client';
import { useQuery, useQueryClient } from '@tanstack/react-query';
import { useState } from 'react';

import { identityQueryKeys } from '@/identity.queries';
import { isStepUpRequiredError } from '@/identity.step-up';
import { type DeviceGroup, groupSessionsByDevice } from './IdentitySessionsPage.device';

export function useIdentitySessionsPage() {
  ...
  const sessionsQueryKey = identityQueryKeys.sessions;
  const securityOverviewQueryKey = identityQueryKeys.securityOverview;
