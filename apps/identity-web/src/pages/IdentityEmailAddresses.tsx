...
import { useIdentityEmailAddresses } from './useIdentityEmailAddresses';

export default function IdentityEmailAddressesPage() {
  const model = useIdentityEmailAddresses();

  return (
    <IdentityPage>
      <IdentityPageHeader
        title="Adresses e-mail"
        description="Gérez les identifiants e-mail utilisés pour vous connecter et récupérer votre compte."
      />
      <IdentityEmailAddresses
        adding={model.adding}
        deletingEmailId={model.deletingEmailId}
        emails={model.emails}
        emailDraft={model.emailDraft}
        error={model.error}
        promotingEmailId={model.promotingEmailId}
        primaryMinAgeHours={model.primaryMinAgeHours}
        resendingEmailId={model.resendingEmailId}
        onAdd={model.handleAdd}
        onDelete={model.handleDelete}
        onDraftChange={model.setEmailDraft}
        onPromote={model.handlePromote}
        onResendVerification={model.handleResendVerification}
      />
    </IdentityPage>
  );
}

export function IdentityEmailAddresses({
  adding,
  deletingEmailId,
  emails,
  emailDraft,
