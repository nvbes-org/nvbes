...
export function useIdentitySecurityPage() {
  ...
  return {
    isPending,
    mutation,
    onOpenMfa: () => startTransition(() => void navigate({ to: '/mfa' })),
    onOpenPassword: () => startTransition(() => void navigate({ to: '/security/password' })),
    onOpenSessions: () => startTransition(() => void navigate({ to: '/sessions' })),
    overview,
  };
}
