...
use crate::worker::{
    BillingWorkerState,
    analytics::capture_billing_webhook_analytics,
...
pub async fn process_stripe_webhook_job(
    ...
    let workspace_id = nvbes_billing::stripe_webhook_processing::process_stripe_event(
        &mut tx,
        &job_payload,
    )
    .await
    ...
    if let Some(workspace_id) = workspace_id {
        ...
        enqueue_billing_email_for_stripe_event(
            &state.db,
            &state.redis,
            workspace_id,
            &job_payload,
