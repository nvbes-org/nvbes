...
use crate::usage_center_actions_test_support::{
    assert_usage_audit_metadata, correction_count, correction_request, correction_request_with_key,
    idempotency_schema_exists, response_json, seed_workspace_with_actor, test_pool,
    usage_actions_schema_exists, usage_audit_count,
};

#[tokio::test]
async fn correct_usage_route_enforces_role_confirmation_grant_and_audits_success() {
    ...
    for role in ["finance_admin", "developer_admin", "operations_admin"] {
        let denied = app
            .clone()
            .oneshot(correction_request(
                workspace_id,
    ...
    assert_eq!(payload["action_kind"], "correct_usage");
    assert_eq!(payload["status"], "applied");

    assert_eq!(correction_count(&pool, tenant_id, actor_id).await, 1);
    assert_usage_audit_metadata(&pool, tenant_id, actor_id, &payload).await;
}

#[tokio::test]
async fn correct_usage_reuses_stored_idempotent_response() {
    let Some(pool) = test_pool().await else {
        eprintln!("skipping test: Postgres is not reachable");
        return;
    };
    ...
    let first = app
        .clone()
        .oneshot(correction_request_with_key(
            workspace_id,
            actor_id,
            "product_admin",
            &confirm_code,
    ...
    assert_eq!(correction_count(&pool, tenant_id, actor_id).await, 1);
    assert_eq!(usage_audit_count(&pool, tenant_id, actor_id).await, 1);
}
