...
pub(crate) async fn billing_platform_actions_schema_exists(pool: &PgPool) -> bool {
...
pub(crate) async fn seed_kyc_profile_with_actor(
    pool: &PgPool,
    actor_id: Uuid,
) -> (Uuid, Uuid, Uuid) {
...
pub(crate) fn disable_routing_request(
    workspace_id: Uuid,
    rule_id: Uuid,
    actor_id: Uuid,
    role: &str,
...
pub(crate) fn approve_request_with_key(
    workspace_id: Uuid,
    profile_id: Uuid,
    actor_id: Uuid,
    role: &str,
...
pub(crate) async fn assert_approved_kyc_state(pool: &PgPool, profile_id: Uuid) {
...
pub(crate) async fn platform_action_count(
    pool: &PgPool,
    tenant_id: Uuid,
    actor_id: Uuid,
    profile_id: Uuid,
...
pub(crate) async fn approve_audit_count(pool: &PgPool, tenant_id: Uuid, actor_id: Uuid) -> i64 {
    sqlx::query_scalar::<_, i64>(
...
pub(crate) async fn assert_approve_audit_metadata(
    pool: &PgPool,
    tenant_id: Uuid,
    actor_id: Uuid,
    profile_id: Uuid,
