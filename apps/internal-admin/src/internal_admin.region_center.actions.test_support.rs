...
pub(crate) async fn region_actions_schema_exists(pool: &PgPool) -> bool {
...
pub(crate) fn flag_request_with_key(
    workspace_id: Uuid,
    actor_id: Uuid,
    role: &str,
    confirm_code: &str,
...
pub(crate) fn flag_request_without_second_approver(
    workspace_id: Uuid,
    actor_id: Uuid,
    role: &str,
    confirm_code: &str,
...
pub(crate) async fn region_action_count(
    pool: &PgPool,
    tenant_id: Uuid,
    actor_id: Uuid,
    workspace_id: Uuid,
...
pub(crate) async fn region_audit_count(pool: &PgPool, tenant_id: Uuid, actor_id: Uuid) -> i64 {
    sqlx::query_scalar::<_, i64>(
