...
pub(crate) async fn usage_actions_schema_exists(pool: &PgPool) -> bool {
...
pub(crate) fn correction_request_with_key(
    workspace_id: Uuid,
    actor_id: Uuid,
    role: &str,
    confirm_code: &str,
...
pub(crate) async fn correction_count(pool: &PgPool, tenant_id: Uuid, actor_id: Uuid) -> i64 {
...
pub(crate) async fn usage_audit_count(pool: &PgPool, tenant_id: Uuid, actor_id: Uuid) -> i64 {
    sqlx::query_scalar::<_, i64>(
...
pub(crate) async fn assert_usage_audit_metadata(
    pool: &PgPool,
    tenant_id: Uuid,
    actor_id: Uuid,
    ...
    assert_eq!(
        audit_metadata["object_links"]["usage_correction_id"],
        payload["object_id"]
    );
    assert_eq!(
        audit_metadata["changes"][1],
        json!({
