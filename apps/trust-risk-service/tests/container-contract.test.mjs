...
const metricsSource = readFileSync(join(serviceRoot, 'src/trust_risk.metrics.rs'), 'utf8');
const deploymentWorkflow = readFileSync(
  join(workspaceRoot, '.github/workflows/deploy.yml'),
  'utf8',
);
const runtimeTerraform = readFileSync(
  join(workspaceRoot, 'infrastructure/environments/trust-risk-production/runtime.tf'),
  'utf8',
);
const observabilityTerraform = readFileSync(
  join(workspaceRoot, 'infrastructure/environments/trust-risk-production/observability.tf'),
  'utf8',
);
...
test('deployment materializes the data-only database identity before runtime validation', () => {
  const deploySection = deploymentWorkflow.slice(deploymentWorkflow.indexOf('deploy-trust-risk:'));
  const foundationStart = deploySection.indexOf('- name: Plan migration foundation');
  const validationStart = deploySection.indexOf(
    '- name: Validate production runtime configuration',
  );
  const foundation = deploySection.slice(foundationStart, validationStart);

  assert.ok(foundationStart >= 0);
  assert.ok(validationStart > foundationStart);
  assert.ok(foundation.includes('-target=scaleway_iam_api_key.trust_risk_database_runtime'));
...
test('deployment can reuse only a signed artifact with unchanged runtime inputs', () => {
  assert.ok(deploymentWorkflow.includes('TRUST_RISK_REUSE_SOURCE_IMAGE_DIGEST'));
  assert.ok(deploymentWorkflow.includes('TRUST_RISK_REUSE_SOURCE_COMMIT_SHA'));
  assert.ok(deploymentWorkflow.includes('git diff --quiet "$SOURCE_COMMIT_SHA" "$GITHUB_SHA" --'));
});
...
test('runtime initializes Sentry and authenticated Grafana OTLP', () => {
  ...
  assert.ok(
    mainSource.includes('otlp_authorization_header: config.otlp_authorization_header.as_deref()'),
  );
