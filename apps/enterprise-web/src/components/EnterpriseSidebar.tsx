...
type EnterpriseNavLabel = (typeof enterpriseNavSections)[number]['items'][number];
...
export function EnterpriseSidebar({ variant = 'sidebar' }: EnterpriseSidebarProps) {
  if (variant === 'mobile') {
    return (
      <nav className="flex gap-2 overflow-x-auto px-3 py-2">
        {enterpriseNavSections.flatMap((section) =>
          section.items.map((label) => <EnterpriseNavItem key={label} label={label} compact />),
        )}
      </nav>
    );
  }
  ...
  return (
    <div className="flex h-full flex-col bg-background text-foreground">
      <div className="border-b border-border px-4 py-4">
        ...
        <p className="mt-0.5 truncate text-xs text-muted-foreground">Tenant administration</p>
      </div>
