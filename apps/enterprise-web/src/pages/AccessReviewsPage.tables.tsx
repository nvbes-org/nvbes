...
function ReminderSummary({
	...
	const total =
		campaign.due_soon_reminders_sent + campaign.overdue_reminders_sent;
	if (total === 0) {
...
export function ReviewItemsTable({
	...
	onDecide: (
		itemId: string,
		decision: AccessReviewDecisionInput["decision"],
	) => void;
}) {
	if (items.length === 0) {
		return (
...
function DecisionActions({
	...
	const canChange =
		(item.item_type === "role" || item.item_type === "service_account") &&
