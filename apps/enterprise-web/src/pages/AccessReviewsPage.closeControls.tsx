...
export function AccessReviewCloseControls({
  ...
  const closeLabel =
    pendingItems > 0 ? `Close with ${pendingItems} pending` : 'Close completed review';
    ...
    >
      ...
      <DialogContent>
        <DialogHeader>
          ...
          <DialogDescription>
            Closing a campaign freezes the review snapshot and prevents further item decisions.
          </DialogDescription>
        </DialogHeader>
        {campaign ? (
          <div className="rounded-md border bg-muted/30 p-3 text-sm">
            ...
            <div className="mt-1 text-muted-foreground">
              {pendingItems} pending, {campaign.approved_items} approved, {campaign.revoked_items}{' '}
              revoked, {campaign.changed_items} changed
            </div>
          </div>
        ) : null}
        {pendingItems > 0 ? (
          <div className="flex gap-2 rounded-md border border-destructive/30 bg-destructive/10 p-3 text-sm text-destructive">
            ...
            <span>
              This campaign still has pending items. They will remain undecided in the exported
              evidence.
            </span>
          </div>
        ) : null}
