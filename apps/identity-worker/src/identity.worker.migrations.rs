...
const MIGRATION_DATABASE_URL_ENV: &str = "NVBES_IDENTITY_MIGRATION_DATABASE_URL";
...
mod tests {
    ...
    fn production_requires_a_dedicated_migration_database_url() {
        let error =
            resolve_migration_database_url("production", "postgres://runtime@db/identity", None)
    ...
    fn production_rejects_the_runtime_database_url() {
        let runtime = "postgres://runtime@db/identity";
        let error =
    ...
    fn production_rejects_the_runtime_role_even_when_the_url_differs() {
        let error = resolve_migration_database_url(
            ...
            Some("postgres://runtime@db/identity?application_name=migrator".to_string()),
        )
    ...
    fn development_can_reuse_the_runtime_database_url() {
        let runtime = "postgres://postgres@localhost/identity";
