...
| sessions incompatibles | Product lead | blocking | mitigated | `apps/account-web/src/account.oauth.client.ts`<br>`apps/account-web/src/account.oauth.access-token.test.ts`<br>`apps/console-web/src/developer.session.ts`<br>`apps/console-web/src/__tests__/developer.session.test.ts` | allowed: incompatible sessions are handled by explicit logout/session clearing and a validated login return flow |
...
```bash
node tools/migration/risk-register.mjs --write
...
```bash
node tools/migration/risk-register.mjs --strict
```
