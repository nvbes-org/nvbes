...
| Check | Status | Path |
|---|---:|---|
| Identity OpenAPI exposes OAuth client listing | passed | `apps/identity-service/openapi.json` |
| Identity OpenAPI exposes OAuth client creation | passed | `apps/identity-service/openapi.json` |
| Identity OpenAPI exposes OAuth client revocation | passed | `apps/identity-service/openapi.json` |
| Identity OpenAPI exposes OAuth client policy creation | passed | `apps/identity-service/openapi.json` |
| Developer OpenAPI exposes token inspection | passed | `apps/developer-service/openapi.json` |
| Identity OpenAPI export includes OAuth client handlers | passed | `apps/identity-service/src/identity.http.openapi.rs` |
| Developer OpenAPI export includes token inspection handler | passed | `apps/developer-service/src/developer.http.openapi.rs` |
| OAuth routes wire list, create, and revoke | passed | `apps/identity-service/src/identity.domains.oauth.routes.clients.rs` |
| OAuth management mutations require recent step-up | passed | `apps/identity-service/src/identity.domains.oauth.routes.clients.rs` |
| OAuth creation returns generated client secret once | passed | `apps/identity-service/src/identity.domains.oauth.clients.create.rs` |
| OAuth revocation invalidates client refresh tokens | passed | `apps/identity-service/src/identity.domains.oauth.clients.revoke.rs` |
| OAuth revocation invalidates pushed authorization requests | passed | `apps/identity-service/src/identity.domains.oauth.clients.revoke.rs` |
| OAuth revocation invalidates authorization codes | passed | `apps/identity-service/src/identity.domains.oauth.clients.revoke.rs` |
| OAuth revocation invalidates device codes | passed | `apps/identity-service/src/identity.domains.oauth.clients.revoke.rs` |
| Machine token test records last-used and audit metadata | passed | `apps/identity-service/src/identity.domains.oauth.flows.client_credentials.tests.rs` |
