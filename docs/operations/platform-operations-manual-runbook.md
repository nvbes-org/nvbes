...
2. Déployer avec `NVBES_IDENTITY_MFA_KEY_VERSION` incrémenté, la nouvelle clé active, et conserver l'ancienne clé dans `NVBES_IDENTITY_MFA_PREVIOUS_ENCRYPTION_KEY` avec `NVBES_IDENTITY_MFA_PREVIOUS_KEY_VERSION`.
3. Exécuter la commande de maintenance `rotate-mfa-key`.
4. Vérifier que le nombre de facteurs migrés correspond aux facteurs actifs de l'ancienne version.
...
3. Mettre à jour `NVBES_IDENTITY_TOKEN_PRIVATE_KEY_PEM` et `NVBES_IDENTITY_TOKEN_PUBLIC_KEY_PEM`.
4. Mettre à jour la clé publique et le `TOKEN_KEY_ID` sur les Resource Servers (`account-service`, etc.).
5. Vérifier que `GET /.well-known/jwks.json` expose la nouvelle clé et que les nouveaux access tokens sont acceptés.

### Révocation d'urgence en cas de compromission de compte

1. Ouvrir un dossier dans Platform Operations (`catégorie: compromission de compte`).
...
3. Révoquer l'ensemble des familles de refresh tokens associées :
   `UPDATE identity_refresh_tokens SET revoked_at = clock_timestamp() WHERE principal_id = $1 AND revoked_at IS NULL;`
4. Déclencher l'envoi d'un email de récupération de mot de passe à l'utilisateur.
