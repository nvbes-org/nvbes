...
| A — serveur HTTP autorisation/token, discovery, JWKS, UserInfo et ID tokens              | ID tokens, JWKS et métadonnées discovery générés dans la bibliothèque ; montage HTTP, autorisation/token et UserInfo restent à livrer                                                            |
| A — consentement, cookies/CSRF/Origin/CORS et throttling                                 | Primitives actives prêtes à être montées : cookie hôte, preuve CSRF/Origin, consentement transactionnel, refus `prompt=none`, limites PostgreSQL bornées ; login et routes HTTP restent à monter |
...
| D — PAR HTTP et consommation atomique                                                    | Endpoint POST `/oauth/par` validant puis stockant une requête opaque ; consommation PostgreSQL atomique et raccordement authorization restant                                                    |
