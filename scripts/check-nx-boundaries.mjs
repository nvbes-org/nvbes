...
import { checkSourceImportBoundaries } from './check-nx-boundaries.imports.mjs';
...
function projectFiles() {
  try {
    const output = execFileSync(
      'rg',
      ['--files', '-g', 'project.json', 'apps', 'libs', 'tools', 'infrastructure'],
      {
        encoding: 'utf8',
    ...
    return projectFilesFromNode(['apps', 'libs', 'tools', 'infrastructure']);
  }
}
...
function errorsForProject(_name, meta) {
  ...
  const infrastructure = type === 'terraform' || type === 'terraform-module';
  if (!layer && !infrastructure) errors.push('missing layer tag');
  if (
    scope &&
    !['product', 'platform', 'cloud', 'internal', 'oss', 'infrastructure'].includes(scope)
  )
    errors.push(`unsupported scope tag ${scope}`);
  if (type && !['app', 'lib', 'workspace', 'terraform', 'terraform-module'].includes(type))
    errors.push(`unsupported type tag ${type}`);
  if (infrastructure && scope !== 'infrastructure')
    errors.push('Terraform projects must use scope:infrastructure');
...
export function addDependencyErrors(source, target, projectMeta, errors) {
...
checkSourceImportBoundaries(projectMeta, packageToProject, addDependencyErrors, dependencyErrors);
