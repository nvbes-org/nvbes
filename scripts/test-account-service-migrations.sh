...
ADMIN_URL="${ACCOUNT_MIGRATION_TEST_ADMIN_URL:-${DATABASE_URL:-${NVBES_DATABASE_URL:-}}}"
if [[ -z "$ADMIN_URL" ]]; then
  echo "Set ACCOUNT_MIGRATION_TEST_ADMIN_URL, DATABASE_URL, or NVBES_DATABASE_URL." >&2
  exit 1
fi

export ACCOUNT_MIGRATION_TEST_ADMIN_URL="$ADMIN_URL"
export ACCOUNT_MIGRATION_TEST_RUN_ID
ACCOUNT_MIGRATION_TEST_RUN_ID="$(
...
finalize_exit() {
  ...
  trap - EXIT INT TERM
  if ! cleanup; then
    echo "Account migration test resource cleanup failed." >&2
    test_status=1
  fi
  exit "$test_status"
}
...
finalize_signal() {
  ...
  trap - EXIT INT TERM
  if ! cleanup; then
    echo "Account migration test resource cleanup failed after a signal." >&2
  fi
  exit "$signal_status"
}
