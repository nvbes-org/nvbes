...
function ensureResourceSecret(directory, resource, encoding = 'base64url') {
  ...
  try {
    try {
      writeFileSync(descriptor, randomBytes(32).toString(encoding));
    } finally {
  ...
  if (bytes.length !== 32 || bytes.toString(encoding) !== secret) {
...
export function ensureResourceSecrets(directory) {
  ...
  ensureResourceSecret(directory, 'billing-authorization', 'hex');
}
