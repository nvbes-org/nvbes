...
const dashboards = [
  {
    ...
    panels: [
      stat(
        ...
        'min by (job) (up{job=~"account-service|cloud-service|backoffice-service|account-worker|cloud-worker"})',
      ),
      graph(
        'HTTP request rate by job/status',
      ...
      graph(
        ...
        'histogram_quantile(0.95, sum by (le, job) (rate(http_request_duration_seconds_bucket[5m]))) or histogram_quantile(0.99, sum by (le, job) (rate(http_request_duration_seconds_bucket[5m])))',
        's',
      ),
      stat(
        'PostgreSQL pool saturation',
        'max((postgres_pool_size - postgres_pool_idle) / clamp_min(postgres_pool_size, 1) * 100)',
        'percent',
      ),
  ...
  },
  {
    ...
    panels: [
      graph(
        ...
        'sum by (job, method) (rate(http_requests_total{job=~"account-service|cloud-service|backoffice-service"}[5m]))',
      ),
      graph(
        'Status mix',
        'sum by (job, status) (rate(http_requests_total{job=~"account-service|cloud-service|backoffice-service"}[5m]))',
      ),
      graph(
        'Latency p95 by service',
        'histogram_quantile(0.95, sum by (le, job) (rate(http_request_duration_seconds_bucket{job=~"account-service|cloud-service|backoffice-service"}[5m])))',
        's',
      ),
      graph(
        'Latency p99 by service',
        'histogram_quantile(0.99, sum by (le, job) (rate(http_request_duration_seconds_bucket{job=~"account-service|cloud-service|backoffice-service"}[5m])))',
        's',
      ),
  ...
  },
  {
    ...
    panels: [
      ...
      graph(
        'OAuth policy denied',
        'sum by (reason) (rate(identity_oauth_policy_denied_total[5m]))',
      ),
      graph(
        'Bot guard decisions',
        'sum by (decision, reason) (rate(identity_bot_guard_decisions_total[5m]))',
      ),
      graph(
        'Authentication outcomes by route',
        'sum by (path, status) (rate(http_requests_total{job="account-service",path=~"/auth/.*"}[5m]))',
      ),
      graph(
        'MFA and WebAuthn errors',
        'sum by (path, status) (rate(http_requests_total{job="account-service",path=~"/auth/.*(mfa|webauthn).*",status=~"4..|5.."}[5m]))',
      ),
    ],
  },
  {
    ...
    panels: [
      graph(
        'Pool size and idle',
        'max by (service, environment) (postgres_pool_size) or max by (service, environment) (postgres_pool_idle)',
      ),
      stat(
        'Pool saturation',
        'max((postgres_pool_size - postgres_pool_idle) / clamp_min(postgres_pool_size, 1) * 100)',
      ...
      graph(
        'Database latency',
        'histogram_quantile(0.95, sum by (le, operation) (rate(postgres_operation_duration_seconds_bucket[5m])))',
        's',
      ),
  ...
  },
  {
    ...
    panels: [
      ...
      graph(
        'Job p95 duration',
        'histogram_quantile(0.95, sum by (le, job_type) (rate(worker_queue_job_duration_seconds_bucket[5m])))',
        's',
      ),
      graph(
        'Recovered jobs',
        'sum by (job_type, outcome) (rate(worker_queue_recovered_jobs_total[5m]))',
      ),
  ...
  simple('nvbes-billing-stripe-webhooks.json', 'nvbes Billing and Stripe Webhooks', 'p0', [
    ...
    graph(
      'Billing operation p95',
      'histogram_quantile(0.95, sum by (le, operation) (rate(billing_operation_duration_seconds_bucket[5m])))',
      's',
    ),
    graph(
      'Provider webhooks',
      'sum by (provider, event_type, outcome) (rate(billing_webhooks_total[5m]))',
    ),
    graph(
      'Webhook p95',
      'histogram_quantile(0.95, sum by (le, provider, event_type) (rate(billing_webhook_duration_seconds_bucket[5m])))',
      's',
    ),
    stat(
      'Webhook failures',
      'sum(increase(billing_webhooks_total{outcome=~"failure|failed"}[1h]))',
    ),
    stat('Ledger drift', 'sum(billing_ledger_drift_total)'),
  ]),
  simple(
    'nvbes-object-storage-upload-download.json',
    'nvbes Object Storage and Upload Download',
    'p0',
    [
      graph('Upload outcomes', 'sum by (operation, outcome) (rate(upload_operations_total[5m]))'),
      graph(
        'Download outcomes',
        'sum by (operation, outcome) (rate(download_operations_total[5m]))',
      ),
      graph(
        'Upload p95 duration',
        'histogram_quantile(0.95, sum by (le, operation) (rate(upload_operation_duration_seconds_bucket[5m])))',
        's',
      ),
      graph(
        'Download p95 duration',
        'histogram_quantile(0.95, sum by (le, operation) (rate(download_operation_duration_seconds_bucket[5m])))',
        's',
      ),
      graph(
        'Object storage operations',
        'sum by (operation, outcome) (rate(object_storage_operations_total[5m]))',
      ),
      graph(
        'Object storage object count',
        'sum by (operation, outcome) (rate(object_storage_objects_total[5m]))',
      ),
    ],
  ),
  simple('nvbes-edge-public-entry.json', 'nvbes Edge and Public Entry', 'p0', [
    ...
    graph(
      'WAF and rate-limit actions',
      'sum by (action, rule) (rate(cloudflare_security_events_total[5m]))',
    ),
  ...
  simple('nvbes-privacy-gdpr-jobs.json', 'nvbes Privacy and GDPR Jobs', 'p0', [
    ...
    graph(
      'Privacy job outcomes',
      'sum by (job_type, outcome) (rate(worker_queue_jobs_total{job_type=~"privacy.*|email.send"}[5m]))',
    ),
    ...
    graph(
      'GDPR email delivery',
      'sum by (outcome) (rate(email_messages_total{business_type=~"data_export|account_delete"}[5m]))',
    ),
  ]),
  simple('nvbes-observability-pipeline.json', 'nvbes Observability Pipeline', 'p0', [
    ...
    graph(
      'Mimir remote write',
      'sum by (status_code) (rate(prometheus_remote_storage_samples_total[5m]))',
    ),
  ...
  ]),
  {
    ...
    panels: [
      ...
      graph(
        'Profile export bytes',
        'sum by (status_code) (rate(pyroscope_write_sent_bytes_total[5m]))',
      ),
      table(
        'CPU profiles',
  ...
  simple('nvbes-logs-correlation.json', 'nvbes Logs and Correlation', 'p1', [
    log(
      'Errors by service',
      'sum by (service) (count_over_time({platform="nvbes"} | json | level=~"error|ERROR" [5m]))',
    ),
    log(
      'Request IDs missing',
      'sum by (service) (count_over_time({platform="nvbes"} | json | request_id="" [5m]))',
    ),
    log(
      'Redaction drops',
      'sum by (service) (count_over_time({platform="nvbes"} |= "redaction" [5m]))',
    ),
  ...
  ]),
  {
    ...
    panels: [
      ...
      logGraph(
        'Frontend signal volume',
        'sum by (kind) (count_over_time({job="account-web"}[5m]))',
      ),
      logGraph(
        'Frontend exceptions',
        'sum(count_over_time({job="account-web",kind="exception"}[5m]))',
      ),
      logGraph(
        'Route changes',
        'sum(count_over_time({job="account-web",kind="event",event_name=~"view_changed|route_change"}[5m]))',
      ),
      logGraph(
        'Consented session starts',
        'sum(count_over_time({job="account-web",kind="event",event_name="session_start"}[5m]))',
      ),
      table(
        'Recent frontend traces',
        '{ resource.service.name = "account-web" }',
        DS.tempo,
        'traceql',
      ),
  ...
  },
  {
    ...
    panels: [
      ...
      graph(
        'Account API request rate',
        'sum by (method, status) (rate(http_requests_total{job="account-service"}[5m]))',
      ),
      graph(
        'Account API p95 latency',
        'histogram_quantile(0.95, sum by (le, path) (rate(http_request_duration_seconds_bucket{job="account-service"}[5m])))',
        's',
      ),
      graph(
        'Account API errors by route',
        'sum by (path, status) (rate(http_requests_total{job="account-service",status=~"4..|5.."}[5m]))',
      ),
      graph(
      ...
      graph(
        'Account worker queue depth',
        'sum by (queue, status) (worker_queue_depth{job="account-worker"})',
      ),
      graph(
        'Account worker outcomes',
        'sum by (job_type, outcome) (rate(worker_queue_jobs_total{job="account-worker"}[5m]))',
      ),
      stat(
        'Account worker heartbeat age',
        'time() - max(worker_heartbeat_timestamp_seconds{job="account-worker"})',
        's',
      ),
      logGraph('Account web signals', 'sum by (kind) (count_over_time({job="account-web"}[5m]))'),
      log(
        'Account runtime errors',
        '{service=~"account-(service|worker)"} | json | level=~"ERROR|error"',
      ),
      table(
        'Recent Account traces',
        '{ resource.service.name =~ "account-(service|worker|web)" }',
        DS.tempo,
        'traceql',
      ),
    ],
  ...
  simple('nvbes-security-abuse.json', 'nvbes Security and Abuse', 'p1', [
    ...
    graph(
      'Public API geo requests',
      'sum by (status, network_kind, risk_bucket) (rate(drive_public_api_geo_requests_total[5m]))',
    ),
    graph(
      'High risk public API requests',
      'sum by (network_kind) (rate(drive_public_api_high_risk_requests_total[5m]))',
    ),
    graph(
      'Network policy blocks',
      'sum by (reason, mode) (rate(drive_public_api_network_policy_blocks_total[5m]))',
    ),
    graph(
      'Internal admin denied',
      'sum by (reason) (rate(internal_admin_permission_denied_total[5m]))',
    ),
  ]),
  simple(
    'nvbes-public-api-developer-platform.json',
    'nvbes Public API and Developer Platform',
    'p1',
    [
      graph(
        'OAuth machine flows',
        'rate(identity_oauth_client_credentials_total[5m]) or rate(identity_oauth_token_exchange_total[5m])',
      ),
      graph('Legacy API key requests', 'rate(drive_api_key_legacy_requests_total[5m])'),
      graph(
        'Identity introspection',
        'rate(drive_identity_introspection_total[5m]) or rate(drive_identity_introspection_cached_total[5m])',
      ),
      graph(
        'Identity introspection p95',
        'histogram_quantile(0.95, sum by (le) (rate(drive_identity_introspection_duration_seconds_bucket[5m])))',
        's',
      ),
      graph(
        'Webhook delivery failures',
        'sum by (endpoint) (rate(developer_webhook_delivery_failures_total[5m]))',
      ),
    ],
  ),
  simple('nvbes-email-communications.json', 'nvbes Email and Communications', 'p1', [
    graph(
      'Email message outcomes',
      'sum by (business_type, outcome) (rate(email_messages_total[5m]))',
    ),
    graph(
      'Provider delivery latency',
      'histogram_quantile(0.95, sum by (le, provider) (rate(email_delivery_duration_seconds_bucket[5m])))',
      's',
    ),
    stat('Queued emails', 'sum(email_queue_depth)'),
    stat('Oldest queued email', 'max(email_queue_oldest_age_seconds)', 's'),
    stat('Suppressed emails', 'sum(email_suppressed_total)'),
    graph(
      'Provider webhook outcomes',
      'sum by (provider, outcome) (rate(email_provider_webhooks_total[5m]))',
    ),
    graph('Expired before dispatch', 'sum(rate(email_expired_total[5m]))'),
  ]),
  simple(
    'nvbes-backoffice-service-operator-safety.json',
    'nvbes Backoffice Operator Safety',
    'p1',
    [
      graph(
        'Admin action requests',
        'sum by (policy, outcome) (rate(internal_admin_action_requests_total[5m]))',
      ),
      graph(
        'Admin action p95',
        'histogram_quantile(0.95, sum by (le, policy) (rate(internal_admin_action_request_duration_seconds_bucket[5m])))',
        's',
      ),
      graph(
        'Guard rejections',
        'sum by (guard, reason) (rate(internal_admin_guard_rejections_total[5m]))',
      ),
      graph(
        'Permission denied',
        'sum by (reason) (rate(internal_admin_permission_denied_total[5m]))',
      ...
      graph(
        'Rate limited',
        'sum by (policy, partition) (rate(internal_admin_rate_limited_total[5m]))',
      ),
    ],
  ),
  simple('nvbes-synthetic-smoke-load-tests.json', 'nvbes Synthetic Smoke and Load Tests', 'p1', [
    ...
    graph(
      'k6 p95 duration',
      'histogram_quantile(0.95, sum by (le, scenario) (rate(k6_http_req_duration_seconds_bucket[5m])))',
      's',
    ),
    stat('k6 checks passing', 'avg(k6_checks_rate)', 'percentunit'),
    stat('Blackbox success', 'avg by (instance) (probe_success)'),
    graph(
      'Production smoke latency',
      'histogram_quantile(0.95, sum by (le, journey) (rate(synthetic_journey_duration_seconds_bucket[5m])))',
      's',
    ),
  ]),
  simple('nvbes-backup-restore.json', 'nvbes Backup and Restore', 'p1', [
    ...
    graph(
      ...
      'histogram_quantile(0.95, sum by (le, target) (rate(restore_test_duration_seconds_bucket[30d])))',
      's',
    ),
  ]),
  ...
  simple('nvbes-product-analytics.json', 'nvbes Product Analytics', 'p2', [
    graph(
      'Activation funnel',
      'sum by (event_name) (increase(product_events_total{event_name=~"signup|workspace_created|first_upload|first_share"}[24h]))',
    ),
    graph(
      'Core Drive usage',
      'sum by (event_name) (increase(product_events_total{event_name=~"file_uploaded|file_downloaded|share_created"}[24h]))',
    ),
    graph('Retention cohorts', 'sum by (cohort) (product_retained_workspaces_total)'),
    graph(
      'Conversion by plan',
      'sum by (plan_code) (increase(product_conversion_events_total[24h]))',
    ),
  ]),
  simple('nvbes-region-data-residency.json', 'nvbes Region and Data Residency', 'p2', [
    ...
    graph(
      'Geo cache latency',
      'histogram_quantile(0.95, sum by (le, provider) (rate(geo_cache_lookup_duration_seconds_bucket[5m])))',
      's',
    ),
    graph(
      'RDAP and IP intelligence',
      'rate(geo_rdap_lookups_total[5m]) or rate(geo_ip_intelligence_lookups_total[5m])',
    ),
  ...
  simple('nvbes-cicd-release-health.json', 'nvbes CI CD and Release Health', 'p2', [
    graph(
      'Deployment outcomes',
      'sum by (environment, service, outcome) (increase(deployments_total[24h]))',
    ),
    stat('Failed migrations', 'sum(increase(postgres_migration_failures_total[24h]))'),
    graph(
      'Post-release smoke',
      'sum by (environment, journey, outcome) (increase(release_smoke_tests_total[24h]))',
    ),
    stat('Rollback signals', 'sum(increase(release_rollbacks_total[7d]))'),
    graph(
      'Release error rate',
      'sum by (release, service) (rate(http_requests_total{status=~"5.."}[5m]))',
    ),
  ]),
];

for (const d of dashboards)
  writeFileSync(join(outDir, d.file), `${JSON.stringify(dashboard(d))}\n`);
