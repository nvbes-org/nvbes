...
#[path = "identity.email.db.delivery.rs"]
mod delivery;
...
pub async fn is_email_suppressed(db: &sqlx::PgPool, email: &str) -> IdentityResult<bool> {
...
pub async fn record_email_message_tx(
    ...
    let message_id = format!("<identity-job-{job_id}@worker.nvbes.fr>");
