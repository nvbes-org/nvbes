...
pub fn hash_client_secret(secret: &str) -> AccountResult<String> {
    use argon2::{Algorithm, Argon2, Params, PasswordHasher, Version, password_hash::SaltString};
    use password_hash::rand_core::OsRng;

    let salt = SaltString::generate(&mut OsRng);
    let params = Params::new(65_536, 3, 4, None)
        .map_err(|error| AccountError::internal("client_secret_hash_failed", error.to_string()))?;
...
pub fn verify_client_secret(secret: &str, hash_value: &str) -> AccountResult<()> {
    use argon2::{Argon2, PasswordVerifier, password_hash::PasswordHash};

    let parsed_hash = PasswordHash::new(hash_value)
        .map_err(|error| AccountError::internal("client_secret_hash_invalid", error.to_string()))?;
    Argon2::default()
        .verify_password(secret.as_bytes(), &parsed_hash)
        .map_err(|_| AccountError::unauthorized("invalid_client", "The client secret is invalid."))
}
