...
export class HttpClient {
	...
	async request<T>(
		...
		const response = await this.fetchImpl(
			url,
			await this.buildInit(url, options),
		);
	...
	private async buildInit(
		...
		applyIdempotencyKey(
			headers,
			method,
			options.idempotencyKey ?? this.idempotencyKey,
		);
		if (
			credentials &&
		...
		const requestE2ee =
			options.requestE2ee === false
				? undefined
				: (options.requestE2ee ?? this.requestE2ee);
