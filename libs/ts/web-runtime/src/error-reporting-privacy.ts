...
export function createErrorReportingReplayPrivacyOptions() {
  return {
    maskAllText: true,
    maskAllInputs: true,
    blockAllMedia: true,
...
export function getErrorReportingTracesSampleRate(isProduction: boolean): number {
  return isProduction ? 0.2 : 1.0;
}

export function getErrorReportingReplaysOnErrorSampleRate(isProduction: boolean): number {
  return isProduction ? 0.1 : 1.0;
}

export function scrubErrorReportingEvent<T>(event: T): T | null {
...
export function scrubErrorReportingBreadcrumb<
