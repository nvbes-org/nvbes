...
const registryPath = 'docs/security/bot-management-controls.json';
...
if (registry) {
  ...
  requirements.forEach((requirement, requirementIndex) => {
    ...
    const requirementId = assertId(requirement?.id, `${requirementContext}.id`, /^BOT_REQ_\d{3}$/u);
    ...
    controls.forEach((control, controlIndex) => {
      ...
      const controlId = assertId(control?.id, `${controlContext}.id`, /^BOT_CTRL_\d{3}$/u);
  ...
  if (errors.length === 0) {
    console.log(
      `Bot management controls: ok (${requirements.length} requirements, ${controlCount} controls, ${evidenceCount} evidence strings)`,
    );
  }
}

if (errors.length > 0) {
  console.error('Bot management controls failed:');
