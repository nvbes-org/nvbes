...
const trustedCacheBranchCondition = "(github.ref == 'refs/heads/main')";
...
function parseWorkflowScope(args) {
  ...
  if (
    ...
    errors.push('usage: check-ci-cd-security.mjs [--workflow .github/workflows/<name>.yml]');
    return undefined;
  }
  return args[1];
}
...
function assertActions(path, text, allowedActions) {
  for (const action of extractUses(text)) {
    ...
    if (!fullCommitShaPattern.test(ref)) {
      errors.push(`${path}: action ${action} must be pinned to a full commit SHA`);
    }
  }
}

function assertSecrets(path, text, allowedSecrets) {
  validateSecrets(path, text, allowedSecrets, {
    errors,
    githubExpression,
...
function assertContinueOnError(path, text, allowedContinueOnError, usedAllowances) {
  ...
  for (const [index, line] of lines.entries()) {
    ...
    for (let cursor = index - 1; cursor >= 0; cursor -= 1) {
      ...
      if (candidateIndent === stepIndent && /^\s*-\s+(?:name|id|uses|run)\s*:/u.test(candidate)) {
        stepStart = cursor;
        break;
      }
    ...
    let stepId;
    if (stepStart >= 0) {
      const idPattern = new RegExp(`^\\s{${fieldIndent}}id\\s*:\\s*([A-Za-z0-9_-]+)\\s*$`, 'u');
...
if (registry) {
  ...
  for (const [index, allowance] of allowedContinueOnError.entries()) {
    const value = requireString(allowance, `${registryPath}.allowedContinueOnError[${index}]`);
    if (value && !/^\.github\/workflows\/[^#\s]+\.ya?ml#[A-Za-z0-9_-]+$/u.test(value)) {
      errors.push(
        `${registryPath}.allowedContinueOnError[${index}]: must be an exact .github/workflows/*.yml#step-id entry`,
      );
    }
    requireUnique(value, allowedContinueOnErrorSet, `${registryPath}.allowedContinueOnError`);
  }
  ...
  requirements.forEach((requirement, requirementIndex) => {
    ...
    controls.forEach((control, controlIndex) => {
      ...
      evidence.forEach((entry, evidenceIndex) => {
        ...
        if (workflowScope) {
          for (const include of requireArray(entry?.includes, `${evidenceContext}.includes`)) {
  ...
  const scopedWorkflowExists = !workflowScope || availableWorkflows.includes(workflowScope);
  ...
  for (const path of workflows) {
    ...
    assertContinueOnError(path, text, allowedContinueOnErrorSet, usedContinueOnErrorAllowances);
    assertCheckout(path, text);
  }

  for (const allowance of allowedContinueOnErrorSet) {
    if (!usedContinueOnErrorAllowances.has(allowance)) {
      errors.push(`${registryPath}.allowedContinueOnError: unused allowance ${allowance}`);
    }
  }
