...
const registryPath = 'docs/security/authentication-controls.json';
...
if (registry) {
  ...
  requirements.forEach((requirement, requirementIndex) => {
    ...
    controls.forEach((control, controlIndex) => {
      ...
      const controlId = assertId(control?.id, `${controlContext}.id`, /^AUTHN_CTRL_\d{3}$/u);
  ...
  if (errors.length === 0) {
    console.log(
      `Authentication controls: ok (${requirements.length} requirements, ${controlCount} controls, ${evidenceCount} evidence strings)`,
    );
  }
}

if (errors.length > 0) {
  console.error('Authentication controls failed:');
