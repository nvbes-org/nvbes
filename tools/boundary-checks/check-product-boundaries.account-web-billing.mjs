...
function checkDedicatedBillingClient(errors) {
  ...
  if (!existsSync(billingClientPath)) {
    errors.push(
      `${billingClientPath}: Account Web must centralize Account-facing Billing transport through a dedicated client`,
    );
    return;
  }

  const content = readFileSync(billingClientPath, 'utf8');
  if (!content.includes('createHttpClient') || !content.includes('VITE_BILLING_SERVICE_BASE_URL')) {
    errors.push(
      `${billingClientPath}: Account Web Billing transport must call Billing Service through a dedicated regional client`,
    );
  }
  if (!content.includes('/workspaces/') || !content.includes('/billing/')) {
    errors.push(
      `${billingClientPath}: Account Web Billing transport must use Billing Service workspace Billing routes`,
    );
  }
  if (content.includes('identityHttpClient') || content.includes('/account/billing/workspaces/')) {
    errors.push(
      `${billingClientPath}: Account Web must not call the removed Account Service Billing facade`,
    );
  }
  if (content.includes('@nvbes/billing-client')) {
    errors.push(
      `${billingClientPath}: Account Web Billing transport must keep its DTO sanitizer before adopting a shared Billing client`,
    );
  }
  if (!content.includes('AccountBillingProviderReferenceSchema')) {
    errors.push(
      `${billingClientPath}: Account Web Billing transport must sanitize provider references before rendering`,
    );
  }
}
  ...
  for (const file of walk('apps/account-web/src')) {
    ...
    for (const forbidden of rawProviderIds) {
      if (content.includes(forbidden)) {
        errors.push(
          `${file}: Account Web must not expose or track raw Billing provider identifier ${forbidden}`,
        );
      }
    }
  }
  ...
  for (const file of walk('apps/account-web/src')) {
    ...
    for (const forbidden of forbiddenRuntimeCopy) {
      if (forbidden.test(content)) {
        errors.push(
          `${file}: Account Web must not expose Billing runtime supervision copy ${forbidden}`,
        );
      }
    }
  }
...
function checkAccountWebDoesNotEmitBillingAnalytics(errors) {
  for (const file of walk('apps/account-web/src')) {
    ...
    if (/trackEvent\(\s*["']billing\./.test(content)) {
      errors.push(
        `${file}: Account Web must not emit Billing analytics events; Billing runtime owns Billing telemetry`,
      );
    }
  }
}
...
function checkAccountWebDoesNotLoadPspVendors(errors) {
  ...
  for (const file of files) {
    ...
    for (const forbidden of forbiddenValues) {
      if (content.includes(forbidden)) {
        errors.push(
          `${file}: Account Web must not load or configure PSP vendor endpoints directly; use Billing service routes`,
        );
      }
    }
  }
...
function checkBillingClientDoesNotExposeRawProviderIds(errors) {
  ...
  for (const file of walk('libs/ts/billing-client/src')) {
    ...
    for (const forbidden of forbiddenProviderIds) {
      if (content.includes(forbidden)) {
        errors.push(
          `${file}: Billing TypeScript client must not expose raw provider identifier ${forbidden}`,
        );
      }
    }
  }
...
function checkIdentityClientDoesNotExposeBillingTransport(errors) {
  ...
  if (existsSync(accountClient)) {
    ...
    if (
      /\/workspaces\/.*\/billing\//.test(content) ||
      /\b(?:getBilling|createBilling)[A-Za-z0-9_]*/.test(content)
    ) {
      errors.push(
        `${accountClient}: AccountIdentityClient must not expose Billing transport methods`,
      );
    }
  }
  ...
  for (const file of walk('libs/ts/identity-client/src')) {
    const content = readFileSync(file, 'utf8');
    if (
      /billing\.(?:client|provider|schemas)/.test(file) ||
      /@nvbes\/billing-client/.test(content)
    ) {
      errors.push(
        `${file}: Identity client package must not own or re-export Billing client surfaces`,
      );
    }
    if (
      /\/enterprise\/billing\b/.test(content) ||
      /\bEnterpriseBilling[A-Za-z0-9_]*\b/.test(content)
    ) {
      errors.push(
        `${file}: Identity client package must not expose Enterprise Billing transport surfaces`,
      );
    }
  }
}
