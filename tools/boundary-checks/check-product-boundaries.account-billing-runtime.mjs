...
function checkAccountRuntimeDoesNotOwnBilling(errors) {
	for (const file of [...walk("apps/account-service/src"), ...walk("apps/account-worker/src")]) {
		...
		if (content.includes("nvbes_billing")) {
			errors.push(`${file}: Account Service must not import nvbes_billing; use billing-service API boundaries instead`);
		}
		if (
		...
		for (const pattern of [
			...
			if (pattern.test(content)) errors.push(`${file}: Account runtime must not own Billing surface ${pattern}`);
		}
	}
...
function checkAccountBetaToolsDoNotOwnBilling(errors) {
	for (const file of walk("apps/account-service/src").filter((path) => path.includes("identity.tools.beta"))) {
		...
		for (const pattern of [
			...
			if (pattern.test(content)) errors.push(`${file}: Account beta tools must stay Account-only and must not own Billing runtime ${pattern}`);
		}
	}
}
...
function checkAccountOpenapiAndClients(errors) {
	...
	if (existsSync(identityOpenapi)) {
		...
		if (/["']\/(?:enterprise|workspaces)\/[^"']*\/billing(?:\/|\{|["'])/.test(content)) {
			errors.push(`${identityOpenapi}: Account OpenAPI must expose Billing only through the Account facade`);
		}
		...
		for (const schema of [
			...
			if (content.includes(`"${schema}"`)) errors.push(`${identityOpenapi}: Account OpenAPI must not expose raw Billing schema ${schema}`);
		}
	}
	...
	for (const file of [
		...
		for (const route of workspaceBillingRouteFragments) {
			if (content.includes(route)) errors.push(`${file}: Account surfaces must not expose workspace Billing route ${route}`);
		}
		if (/\/workspaces\/\$\{[^}]+\}\/billing\//.test(content)) errors.push(`${file}: Account clients must not construct workspace Billing route templates`);
