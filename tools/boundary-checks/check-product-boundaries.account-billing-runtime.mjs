...
export function checkAccountBillingRuntimeBoundary(errors) {
  checkAccountRuntimeDoesNotOwnBilling(errors);
...
function checkAccountRuntimeDoesNotOwnBilling(errors) {
  for (const file of [...walk('apps/account-service/src'), ...walk('apps/account-worker/src')]) {
    ...
    if (content.includes('nvbes_billing')) {
      errors.push(
        `${file}: Account Service must not import nvbes_billing; use billing-service API boundaries instead`,
      );
    }
    if (
      file.endsWith('identity.domains.mod.rs') &&
      (content.includes('identity.domains.billing.mod.rs') ||
        /\bpub\s+mod\s+billing\b/.test(content))
    ) {
    ...
    for (const pattern of [
      ...
      if (pattern.test(content))
        errors.push(`${file}: Account runtime must not own Billing surface ${pattern}`);
    }
  }
  for (const manifest of ['apps/account-service/Cargo.toml', 'apps/account-worker/Cargo.toml']) {
    if (existsSync(manifest) && readFileSync(manifest, 'utf8').includes('nvbes-billing')) {
      errors.push(`${manifest}: Account runtime must not depend on nvbes-billing`);
    }
  }
  checkAccountBetaToolsDoNotOwnBilling(errors);
  checkAccountOpenapiAndClients(errors);
}

function checkAccountBetaToolsDoNotOwnBilling(errors) {
  for (const file of walk('apps/account-service/src').filter((path) =>
    path.includes('identity.tools.beta'),
  )) {
    const content = readFileSync(file, 'utf8');
    for (const pattern of [
      ...
      if (pattern.test(content))
        errors.push(
          `${file}: Account beta tools must stay Account-only and must not own Billing runtime ${pattern}`,
        );
    }
  }
}

function checkAccountOpenapiAndClients(errors) {
  ...
  if (existsSync(identityOpenapi)) {
    ...
    if (
      /["']\/(?:account\/billing|enterprise\/billing|workspaces\/[^"']*\/billing)(?:\/|\{|["'])/.test(
        content,
      )
    ) {
      errors.push(`${identityOpenapi}: Account OpenAPI must not expose Billing routes`);
    }
    if (
      /"name"\s*:\s*"(?:billing|account-billing)"/i.test(content) ||
      /"tags"\s*:\s*\[[^\]]*"billing"/i.test(content)
    ) {
      errors.push(`${identityOpenapi}: Account OpenAPI must not expose raw Billing tags`);
    }
    for (const schema of [
      'BillingAccountView',
      ...
      if (content.includes(`"${schema}"`))
        errors.push(
          `${identityOpenapi}: Account OpenAPI must not expose raw Billing schema ${schema}`,
        );
    }
  }
  ...
  for (const file of [
    ...
    for (const route of workspaceBillingRouteFragments) {
      if (content.includes(route))
        errors.push(`${file}: Account surfaces must not expose workspace Billing route ${route}`);
    }
    if (/\/workspaces\/\$\{[^}]+\}\/billing\//.test(content))
      errors.push(`${file}: Account clients must not construct workspace Billing route templates`);
    if (
      /\b(?:get|create|list|download)Billing(?:Overview|Usage|Entitlements|Checkout|Portal|Invoices|Cards|Subscriptions|InvoicePdf)\b/.test(
        content,
      )
    ) {
      errors.push(`${file}: Account clients must not expose workspace Billing transport methods`);
    }
  }
}
