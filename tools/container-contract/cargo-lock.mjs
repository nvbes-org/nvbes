...
function parseLockPackages(source) {
  ...
  for (const block of source.split(/(?=^\[\[package\]\]\s*$)/m)) {
    ...
    assert.ok(name && version, 'every Cargo.lock package must have a name and version');
    const dependencyBlock = block.match(/^dependencies\s*=\s*\[([\s\S]*?)^\]/m)?.[1] ?? '';
...
function parseLockDependency(specification) {
  const parenIndex = specification.indexOf(' (');
  const withoutSource = parenIndex >= 0 ? specification.slice(0, parenIndex) : specification;
  const match = withoutSource.match(/^([^\s]+)\s+([0-9A-Za-z.-]+)$/);
  return match
...
export function lockedPackageGraphSnapshot(lockfile, rootName) {
  ...
  assert.equal(roots.length, 1, `Cargo.lock must contain exactly one ${rootName} package`);
  ...
  while (pending.length > 0) {
    ...
    for (const specification of pkg.dependencies) {
      ...
      const candidates = (byName.get(dependency.name) ?? []).filter(
        (candidate) => !dependency.version || candidate.version === dependency.version,
      );
      assert.equal(
        candidates.length,
        1,
  ...
  const packageIds = [...visited.keys()].sort((a, b) => a.localeCompare(b));
...
export function offlineMetadataMsrv(workspaceRoot, rootName) {
  ...
  while (pending.length > 0) {
    ...
    if (pkg?.rust_version && (!msrv || compareVersions(pkg.rust_version, msrv) > 0)) {
