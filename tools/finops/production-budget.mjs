...
export function validateBudgetContract(contract) {
  ...
  if (!Array.isArray(monthly.alertsCents) || monthly.alertsCents.length === 0) {
    ...
    for (const requiredAlertCents of REQUIRED_ALERTS_CENTS) {
      if (!monthly.alertsCents.includes(requiredAlertCents)) {
        errors.push(`alerts must include the required ${requiredAlertCents}-cent threshold`);
      }
    }
  ...
  if (!Array.isArray(contract.stages) || contract.stages.length !== REQUIRED_STAGES.length) {
    ...
    if (
      ...
      errors.push('essential_only stage must start no later than monthly hard limit');
    }
  }
...
export async function loadBudgetContract(file) {
  ...
  if (rel.startsWith('..') || path.isAbsolute(rel)) {
    throw new Error('Path traversal: budget contract file must be within workspace');
  }
