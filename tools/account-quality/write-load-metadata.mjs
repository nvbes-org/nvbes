...
if (!outputPath || !summaryName) {
	throw new Error(
		"Usage: node write-load-metadata.mjs <output-path> <summary-name>",
	);
}
...
const metadata = {
	configuredDuration: profileDuration(
		process.env.K6_PROFILE,
		process.env.ACCOUNT_K6_DURATION,
	),
...
function releaseIdentifier() {
	...
	if (
		process.env.NVBES_TARGET_ENV === "staging" &&
		!/^[a-f0-9]{7,64}$/u.test(raw)
	) {
		throw new Error(
			"NVBES_RELEASE must be an immutable hexadecimal revision in staging.",
		);
	}
	if (
		process.env.NVBES_TARGET_ENV !== "staging" &&
		!/^[a-z0-9][a-z0-9._-]{0,79}$/iu.test(raw)
	) {
