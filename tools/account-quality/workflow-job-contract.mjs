...
const weeklyCondition =
	"github.event_name == 'schedule' && github.event.schedule == '31 1 * * 0'";
...
const jobContract = {
	...
	"manual-resilience": {
		...
		condition:
			"github.event_name == 'workflow_dispatch' && inputs.lane == 'resilience'",
...
export function validateWorkflowJobs(jobs) {
	...
	assert(
		jobs["nightly-load"].env?.K6_PROFILE === "load",
		"nightly load must use profile load",
	);
	...
	assert(
		uploadCount === 4,
		"workflow must contain exactly four evidence upload steps",
	);
}
...
function validateCampaignGate(job) {
	...
	const failClosed = requireNamedStep(
		job,
	...
	const validation = requireNamedStep(
		job,
		"Validate lane, exact targets and duration",
	);
	for (const command of [
		...
		assert(
			validation.run?.includes(command),
			`campaign-gate is missing ${command}`,
		);
	}
}
...
function validateCampaignJob(jobName, job, contract) {
	...
	assertDeepEqual(
		job.needs,
		["campaign-gate", "quality-contract"],
		`${jobName} dependencies`,
	);
	assert(
		job.if === contract.condition,
		`${jobName} trigger condition differs from its contract`,
	);
	requireRunStep(
		job,
		"CI/CD security gate",
...
function validateArtifact(jobName, job, artifact) {
	...
	assert(
		upload.length === 1,
		`${jobName} must contain exactly one evidence upload`,
	);
	assert(
		upload[0].if === "always()",
		`${jobName} evidence must upload even after failure`,
	);
	assertDeepEqual(
		upload[0].with,
		{
...
function validateWeeklyMatrix(job) {
	assert(
		...
		"weekly matrix must collect every profile result",
	);
	assert(
		job.strategy?.["max-parallel"] === 1,
		"weekly matrix must be serialized",
	);
	assertDeepEqual(
		job.strategy?.matrix,
