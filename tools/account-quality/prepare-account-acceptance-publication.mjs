...
export async function prepareAccountAcceptancePublication({
	sourceRoot,
	publicationRoot,
}) {
	const canonicalSourceRoot = await requireDirectory(
		sourceRoot,
		"source evidence",
	);
...
function publicationFiles(manifest) {
	assert(
		Array.isArray(manifest.reports) &&
			manifest.reports.length === REQUIRED_HUMAN_CATEGORIES.length,
		"acceptance manifest must reference exactly 12 reports",
	);
	assert(
		isRecord(manifest.deployment),
		"acceptance manifest deployment reference is required",
	);
	...
	for (const report of manifest.reports) {
		...
		assert(
			...
			`duplicate acceptance report ${report.category}`,
		);
...
async function walkEvidenceTree(root) {
	...
	async function visit(relativeDirectory, depth) {
		...
		for (const name of await readdir(directory)) {
			...
			assert(
				entries <= MAX_ENTRIES,
				"acceptance evidence has too many entries",
			);
			assert(
			...
			if (metadata.isDirectory()) {
				...
				assert(
					...
					"acceptance evidence exceeds the total size limit",
				);
...
function assertExactTree(observed, allowedFiles) {
	const observedFiles = [...observed.files.keys()].sort();
	const expectedFiles = [...allowedFiles].sort();
	assert(
	...
	const observedDirectories = [...observed.directories].sort();
	const allowedDirectories = [...expectedDirectories].sort();
	assert(
		observedDirectories.length === allowedDirectories.length &&
			observedDirectories.every(
				(value, index) => value === allowedDirectories[index],
			),
...
async function main() {
	const [, , sourceRoot, publicationRoot] = process.argv;
	if (!sourceRoot || !publicationRoot) {
		throw new Error(
			"usage: prepare-account-acceptance-publication.mjs <source> <publication>",
		);
	}
