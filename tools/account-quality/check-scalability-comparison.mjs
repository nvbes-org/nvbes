...
const comparisonPath = process.argv[2] || process.env.ACCOUNT_SCALABILITY_COMPARISON_FILE;
if (!comparisonPath) {
  throw new Error('ACCOUNT_SCALABILITY_COMPARISON_FILE is required.');
}

const absoluteComparisonPath = resolveComparisonPath(comparisonPath);
...
for (const run of comparison.runs) {
  ...
  assert(summary.setup_data == null, 'scalability evidence must not contain setup_data');
  ...
  assert(errorRate < 0.005, `replica ${replicas} run exceeded the error-rate SLO`);
  assert(checkRate > 0.995, `replica ${replicas} run exceeded the check-failure SLO`);
...
for (const run of evaluatedRuns.slice(1)) {
  for (const key of sharedKeys) {
    assert(run.metadata[key] === baseline.metadata[key], `scalability ${key} must be identical`);
  }
...
function resolveComparisonPath(rawPath) {
  if (/[\0\r\n]/u.test(rawPath) || rawPath.split(/[\\/]+/u).includes('..')) {
    throw new Error('comparison path must not contain traversal segments or control characters');
  }
  return path.resolve(process.cwd(), rawPath);
}

function assertVerdictPathSafe(candidatePath) {
  const resolvedVerdictPath = path.resolve(comparisonRoot, candidatePath);
  const relativeVerdictPath = path.relative(comparisonRoot, resolvedVerdictPath);
  if (relativeVerdictPath.startsWith('..') || path.isAbsolute(relativeVerdictPath)) {
    throw new Error('scalability verdict path must stay inside the evidence directory');
  }
...
async function readJson(relativePath, label) {
  assert(typeof relativePath === 'string' && relativePath !== '', `${label} path is required`);
  ...
  if (relativeResolvedPath.startsWith('..') || path.isAbsolute(relativeResolvedPath)) {
    throw new Error(`${label} path must stay inside the evidence directory`);
  }
...
function metric(summary, metricName, valueName) {
  ...
  assert(typeof value === 'number' && Number.isFinite(value), `${metricName}.${valueName} missing`);
  return value;
}
