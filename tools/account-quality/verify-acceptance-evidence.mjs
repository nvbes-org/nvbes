...
export const REPORT_MAX_AGE_MS = Object.freeze({
...
export async function verifyAcceptanceEvidence({
  ...
  assert(RELEASE_PATTERN.test(expectedRelease ?? ''), 'expected release must be an immutable SHA');
  assert(
  ...
  const signature = await readBoundedRegularFile(signaturePath, 4096, 'detached signature');
  const publicKey = await readBoundedRegularFile(publicKeyPath, 16_384, 'trusted public key');
  assert(
  ...
  const issuedAt = validateEnvelope(manifest, expectedRelease, expectedSignerKeyId, now);
...
function validateEnvelope(manifest, expectedRelease, expectedSignerKeyId, now) {
  assert(manifest.schemaVersion === 1, 'unsupported acceptance evidence schema');
  assert(manifest.environment === 'staging', 'acceptance evidence must come from staging');
  assert(manifest.release === expectedRelease, 'acceptance evidence release mismatch');
  assert(
  ...
  const issuedAt = canonicalTimestamp(manifest.issuedAt, 'issuedAt');
  const expiresAt = canonicalTimestamp(manifest.expiresAt, 'expiresAt');
  const nowMs = now.getTime();
  assert(issuedAt <= nowMs + 5 * 60 * 1000, 'acceptance evidence is future-dated');
  ...
  assert(expiresAt - issuedAt <= MAX_VALIDITY_MS, 'acceptance evidence validity exceeds 31 days');
  assert(Array.isArray(manifest.reports), 'acceptance evidence reports must be an array');
  return issuedAt;
}
...
async function validateReports(
  ...
  for (const report of reports) {
    ...
    assert(report.release === expectedRelease, `${report.category} report release mismatch`);
    assert(report.environment === environment, `${report.category} report environment mismatch`);
    assert(report.decision === 'passed', `${report.category} decision must be passed`);
    assert(nonEmpty(report.signedBy), `${report.category} must name its human signer`);
    const signedAt = canonicalTimestamp(report.signedAt, `${report.category}.signedAt`);
    assert(signedAt <= issuedAt, `${report.category} was signed after issuance`);
    assert(signedAt <= now.getTime() + 5 * 60 * 1000, `${report.category} is future-dated`);
    assert(
    ...
    if (report.category === 'penetration') {
      assert(report.independent === true, 'penetration evidence must be independent');
    }
...
async function verifyArtifact(entry, evidenceRoot, label) {
  ...
  assert((await sha256File(artifactRealPath)) === entry.sha256, `${label} digest mismatch`);
...
async function main() {
  const result = await verifyAcceptanceEvidence({
    ...
    deploymentTrust: {
      ...
      expectedPublicKeySha256: process.env.ACCOUNT_DEPLOYMENT_TRUSTED_PUBLIC_KEY_SHA256,
    ...
    expectedPublicKeySha256: process.env.ACCOUNT_ACCEPTANCE_TRUSTED_PUBLIC_KEY_SHA256,
...
function parseExpectedComponentUids(source) {
  ...
  try {
    ...
    throw new Error('ACCOUNT_DEPLOYMENT_EXPECTED_COMPONENT_UIDS must be valid JSON');
  }
}
