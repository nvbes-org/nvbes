...
export function parseWorkflow(workflowSource) {
  ...
  try {
    ...
    throw new Error(`account-quality workflow is invalid YAML: ${error.message}`, { cause: error });
  }
...
function validateTriggers(triggers, automation) {
  ...
  assertExactKeys(triggers, ['schedule', 'workflow_dispatch'], 'workflow triggers');
  assert(Array.isArray(triggers.schedule), 'workflow schedule must be an array');
  assertDeepEqual(
  ...
  assertExactKeys(dispatch.inputs, Object.keys(inputContract), 'workflow_dispatch inputs');
...
function validateInput(name, actual, expected) {
  ...
  assert(nonEmpty(actual.description), `workflow input ${name} requires a description`);
  const actualContract = Object.fromEntries(Object.keys(expected).map((key) => [key, actual[key]]));
  assertDeepEqual(actualContract, expected, `workflow input ${name}`);
  assertExactKeys(actual, ['description', ...Object.keys(expected)], `workflow input ${name}`);
}
...
function validateGlobalControls(workflow) {
  ...
  assert(workflow.env?.CI === 'true', 'workflow must define CI as the string true');
  assert(
    workflow.defaults?.run?.shell === 'bash --noprofile --norc -euo pipefail {0}',
