...
export function selectAffectedCargoPackages(metadata, changedPaths, workspace) {
  const packages = metadata.packages.filter((item) => metadata.workspace_members.includes(item.id));
  const allNames = packages.map(({ name }) => name).sort((a, b) => a.localeCompare(b));
  if (
    changedPaths.some(
      (path) =>
  ...
  for (const path of changedPaths) {
    ...
    const owner = roots
      .filter(({ root }) => path === `${root}/Cargo.toml` || path.startsWith(`${root}/`))
  ...
  while (changed) {
    ...
    for (const item of packages) {
      ...
      if (item.dependencies.some((dependency) => selected.has(dependency.name))) {
