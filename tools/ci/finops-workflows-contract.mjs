...
const SETUP_TERRAFORM_ACTION = 'hashicorp/setup-terraform@dfe3c3f87815947d99a8997f908cb6525fc44e9e';
const COREPACK_COMMAND = 'corepack enable\ncorepack prepare pnpm@11.18.0 --activate';
const LOCKED_INSTALL_COMMAND = 'pnpm install --frozen-lockfile --prefer-offline';
...
function isFailClosed(value, allowedIf) {
  return (
    ...
    (!Object.hasOwn(value, 'continue-on-error') || value['continue-on-error'] === false)
  );
}
...
function assertCriticalSteps(entries, jobName) {
  assert.equal(
    entries.every(
      ({ step }) => isRecord(step) && isFailClosed(step) && !Object.hasOwn(step, 'shell'),
    ),
    true,
...
function assertGatePrefix(gate, contract) {
  ...
  assert.deepEqual(prefix, expected, `${contract.job} must use the allowlisted gate step prefix`);
}
...
function assertExecutionContext(workflow, gate, contract) {
  ...
  assert.equal(Object.hasOwn(workflow, 'env'), false, 'workflow env is forbidden');
  assert.deepEqual(gate.env, contract.gateEnv, `${contract.job} env must match`);
  assert.deepEqual(gate['runs-on'], contract.runsOn, `${contract.job} runner labels must match`);
  assert.equal(
...
function assertJobInventory(workflow, contract) {
  ...
  const actualJobs = Object.keys(workflow.jobs).sort((a, b) => a.localeCompare(b));
  const matches =
    actualJobs.length === expectedJobs.length &&
    actualJobs.every((jobName, index) => jobName === expectedJobs[index]);
  if (!matches) {
    assert.fail(`workflow jobs must exactly match contract: ${expectedJobs.join(', ')}`);
  }
}
...
export function validateWorkflowContract(source, contract) {
  ...
  assertFailClosedGateJob(gate, contract.job, contract.shell, contract.allowedIf);
  ...
  assert.equal(finOpsSteps.length, 1, `${contract.job} must invoke pnpm check:finops exactly once`);
  ...
  const terraformSteps = gateRunSteps.filter(
    ({ run, stepIndex }) => run === terraformValidationCommand(contract) && stepIndex > 5,
  );
  assert.equal(terraformSteps.length, 1, 'Terraform validation must run after pnpm check:finops');
