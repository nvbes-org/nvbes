...
const emailTerraformEnvironmentPath = 'infrastructure/environments/email-production';
...
const deployContract = {
  ...
  terraformValidationStepName: 'Pre-deploy: Validate isolated email Terraform stack',
...
function withStepProperty(steps, command, property) {
  return steps.replace(`        run: ${command}`, `        run: ${command}\n        ${property}`);
}
...
for (const [scenario, gateProperty] of [
  ...
  test(`rejects a FinOps gate job with ${scenario}`, () => {
    assert.throws(
      () => validateWorkflowContract(deployWorkflow({ gateProperty }), deployContract),
...
test('rejects explicit false continue-on-error values on critical steps', () => {
  ...
  for (const command of [lockedInstall, 'pnpm check:finops', emailTerraformValidation]) {
...
test('rejects a Terraform mention without a real Terraform command after the gate', () => {
  const steps = validGateSteps.replace(emailTerraformValidation, 'echo terraform');
  assert.throws(
    () => validateWorkflowContract(deployWorkflow({ steps }), deployContract),
    /must use the allowlisted gate step prefix/u,
  );
...
${validDeployNeeds}`;
  assert.throws(
    () => validateWorkflowContract(deployWorkflow({ steps, downstream }), deployContract),
...
test('rejects a locked install whose failure is ignored', () => {
  const steps = validGateSteps.replace(lockedInstall, `${lockedInstall} || true`);
  assert.throws(
    () => validateWorkflowContract(deployWorkflow({ steps }), deployContract),
    /must use the allowlisted gate step prefix/u,
  );
...
for (const [scenario, downstream, error] of downstreamFailures) {
  test(`rejects ${scenario}`, () => {
    assert.throws(
      () => validateWorkflowContract(deployWorkflow({ downstream }), deployContract),
      error,
    );
  });
}
