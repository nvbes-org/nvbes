...
test('the production ledger accepts bounded operational validation messages', () => {
  ...
  assert.match(migration, /ALTER TYPE email_category ADD VALUE IF NOT EXISTS 'operational';/u);
});
...
test('email production deploy is isolated and uses an immutable signed image', () => {
  ...
  assert.equal(existsSync('infrastructure/environments/production/email-database.tf'), false);
  assert.equal(existsSync('infrastructure/environments/production/email-delivery.tf'), false);
  const handoff = read('infrastructure/environments/production/email-state-handoff.tf');
  ...
  assert.match(workflow, /EMAIL_DEPLOY_APPROVED_SHA: \$\{\{ vars\.EMAIL_DEPLOY_APPROVED_SHA \}\}/u);
  assert.match(workflow, /\[\[ "\$EMAIL_DEPLOY_CONFIRMATION" == "deploy-email-production" \]\]/u);
  assert.match(workflow, /\[\[ "\$EMAIL_DEPLOY_APPROVED_SHA" =~ \^\[0-9a-f\]\{40\}\$ \]\]/u);
  assert.match(workflow, /\[\[ "\$EMAIL_DEPLOY_APPROVED_SHA" == "\$GITHUB_SHA" \]\]/u);
  assert.match(workflow, /nvbes-org\/nvbes-email-worker:\$\{\{ github\.sha \}\}/u);
  assert.doesNotMatch(
    workflow,
  ...
  assert.match(workflow, /-target=scaleway_job_definition\.email_database_migration/u);
  ...
  assert.match(workflow, /TF_VAR_email_sentry_dsn: \$\{\{ secrets\.EMAIL_SENTRY_DSN \}\}/u);
  assert.match(
    workflow,
  ...
  assert.match(workflow, /\[\[ "\$EMAIL_SYNTHETIC_SMOKE_ENABLED" == "true" \]\]/u);
  assert.match(
    workflow,
  ...
  assert.match(workflow, /if: \$\{\{ vars\.EMAIL_SYNTHETIC_SMOKE_ENABLED == 'true' \}\}/u);
  assert.match(
    workflow,
  ...
  assert.match(workflow, /\.delivery_state == "delivered" and \.processed_provider_events > 0/u);
  assert.match(workflow, /output -raw email_database_runtime_url/u);
  assert.match(workflow, /--env NVBES_EMAIL_DATABASE_URL/u);
  assert.match(
    workflow,
    /name: 'Post-deploy: Close the bounded Email validation window'[\s\S]*?if: \$\{\{ always\(\) \}\}[\s\S]*?TF_VAR_email_internal_validation_enabled=false[\s\S]*?-refresh=false/u,
  );
  assert.match(
    workflow,
  ...
  assert.match(workflow, /terraform[\s\S]*?plan[\s\S]*?-out=email-runtime\.tfplan/u);
  assert.match(workflow, /terraform[\s\S]*?apply[\s\S]*?email-runtime\.tfplan/u);
  ...
  assert.match(delivery, /resource "scaleway_registry_namespace" "email_worker"/u);
  ...
  assert.match(delivery, /private_network_id\s*=\s*[^\n]*var\.private_network_id/u);
  ...
  assert.match(outputs, /output "email_database_runtime_url"[\s\S]*?sensitive\s*=\s*true/u);
});
...
test('container release publishes the image name consumed by Terraform', () => {
  ...
  const email = deployables.rust.find((application) => application.project === 'email-worker');
  assert.equal(email.image, 'nvbes-email-worker');
});

test('email CI validates the isolated deployment stack', () => {
  const workflow = read(workflowPath);
  const continuousIntegration = read('tools/ci/run-lane.mjs');
  const emailEnvironmentProject = read('infrastructure/environments/email-production/project.json');
  const emailStackProject = read('infrastructure/stacks/email/production/project.json');
  ...
  assert.match(continuousIntegration, /nx\(['"]test:contract['"], \[['"]ci-contracts['"]\]/u);
  assert.doesNotMatch(workflow, /pnpm test:unit/u);
  assert.match(workflow, /name: Generate CycloneDX SBOM[\s\S]*?format: cyclonedx(?:\s|$)/u);
