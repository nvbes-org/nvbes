...
function validateGeneration(ledger) {
	...
	if (generation.command !== "node tools/migration/phase-ledger.mjs --write") {
	...
	if (generation.strict_cutover_command !== "node tools/migration/phase-ledger.mjs --strict") {
...
const ledger = {
	schema_version: 1,
	generation: {
		command: "node tools/migration/phase-ledger.mjs --write",
		source: sourcePath,
		strict_cutover_command: "node tools/migration/phase-ledger.mjs --strict",
	},
...
for (const [path, expected] of [[outputPath, json], [markdownPath, markdown]]) {
	if (!existsSync(path)) errors.push(`${path}: missing; run node tools/migration/phase-ledger.mjs --write`);
	else if (readFileSync(path, "utf8") !== expected) errors.push(`${path}: stale; run node tools/migration/phase-ledger.mjs --write`);
}
