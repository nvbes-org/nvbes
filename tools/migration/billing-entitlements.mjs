...
const sources = {
	...
	identityRoutes: "apps/identity-api/src/identity.domains.billing.routes.manage.rs",
	identityOverview: "apps/identity-api/src/identity.domains.billing.service.overview.rs",
	identityEntitlements: "apps/identity-api/src/identity.domains.billing.entitlements.rs",
	identityPolicy: "apps/identity-api/src/identity.domains.billing.policy.rs",
	identityTypes: "apps/identity-api/src/identity.domains.billing.service.types.rs",
	driveCore: "apps/drive-api/src/drive.domains.billing.manage.core.rs",
	driveEntitlements: "apps/drive-api/src/drive.domains.billing.entitlements.rs",
	driveTypes: "apps/drive-api/src/drive.domains.billing.types.rs",
...
function buildChecks() {
	return [
		...
		textCheck("identity-overview", sources.identityOverview, "Identity billing overview returns entitlements", "entitlements: entitlements_view(&record)"),
		textCheck("identity-usage-ledger", sources.identityOverview, "Identity usage response exposes billable storage and seats", "billable_quantity"),
		textCheck("identity-entitlements-route", sources.identityRoutes, "Identity exposes billing entitlements endpoint", "path = \"/workspaces/{workspaceId}/billing/entitlements\""),
		textCheck("identity-entitlements-authz", sources.identityRoutes, "Identity entitlement endpoint requires billing authorization", "WorkspaceAction::ViewBilling"),
		textCheck("identity-entitlements-service", sources.identityEntitlements, "Identity service returns workspace entitlements", "get_entitlements_for_workspace"),
		textCheck("identity-type-reexport", sources.identityTypes, "Identity service re-exports shared entitlement type", "ProductEntitlementsView"),
		textCheck("identity-policy-lock", sources.identityPolicy, "Identity billing operations enforce lock policy", "subscription_status_requires_lock(&billing_record.subscription_status)"),
		textCheck("drive-overview", sources.driveCore, "Drive billing overview returns shared entitlements", "entitlements: entitlements_view(&record)"),
		textCheck("drive-usage-ledger", sources.driveCore, "Drive usage response exposes billable storage and seats", "billable_quantity"),
		textCheck("drive-invoice-persist", sources.driveCore, "Drive persists invoice estimates for reconciliation", "persist_invoice_estimate(db, &estimate)"),
