...
function buildChecks() {
	return [
		...
		textCheck("release-staging-e2e", sources.releaseGate, "Release gate runs authenticated Account acceptance checks", "staging authenticated Account acceptance gate"),
		textCheck("migration-backup-guard", sources.migrateStaging, "Staging migration requires restore point confirmation", "apres avoir verifie le point de restauration Staging"),
		textCheck("smoke-staging", sources.smokeStaging, "Staging smoke wrapper runs smoke and authenticated Account acceptance", "staging authenticated Account acceptance"),
