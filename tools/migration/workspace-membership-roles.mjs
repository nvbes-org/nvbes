...
const sources = {
	...
	authzDb: "apps/identity-service/src/identity.domains.authz.db.rs",
	authzService: "apps/identity-service/src/identity.domains.authz.service.rs",
	authzRoutes: "apps/identity-service/src/identity.domains.authz.routes.rs",
...
function buildChecks() {
	return [
		textCheck(
			"owner-role",
			sources.coreRole,
			"Owner role is part of the workspace role model",
			"WorkspaceRole::Owner",
		),
		textCheck(
			"admin-role",
			sources.coreRole,
			"Admin role is part of the workspace role model",
			"WorkspaceRole::Admin",
		),
		textCheck(
			"security-admin-role",
			sources.coreRole,
			"Security admin role is part of the workspace role model",
			"WorkspaceRole::SecurityAdmin",
		),
		textCheck(
			"billing-admin-role",
			sources.coreRole,
			"Billing admin role is part of the workspace role model",
			"WorkspaceRole::BillingAdmin",
		),
		textCheck(
			"member-role",
			sources.coreRole,
			"Member role is part of the workspace role model",
			"WorkspaceRole::Member",
		),
		textCheck(
			"viewer-role",
			sources.coreRole,
			"Viewer role is part of the workspace role model",
			"WorkspaceRole::Viewer",
		),
		textCheck(
			"policy-entrypoint",
			sources.corePolicy,
			"Workspace role policy uses the shared is_allowed entrypoint",
			"pub fn is_allowed",
		),
		textCheck(
			"owner-policy",
		...
		textCheck(
			...
			sources.corePolicy,
			"Security admin policy is explicit",
			"fn security_admin_allows",
		),
		textCheck(
			"billing-admin-policy",
			sources.corePolicy,
			"Billing admin policy is explicit",
			"fn billing_admin_allows",
		),
		textCheck(
			"member-policy",
			sources.corePolicy,
			"Member policy is explicit",
			"fn member_allows",
		),
		textCheck(
			"viewer-policy",
			sources.corePolicy,
			"Viewer policy is explicit",
			"fn viewer_allows",
		),
		textCheck(
			"role-boundary-test",
			sources.corePolicyTests,
			"Core test covers owner/admin/security/billing/member/viewer permission boundaries",
			"workspace_membership_roles_cover_expected_permission_boundaries",
		),
		textCheck(
			"admin-invite-test",
			sources.corePolicyTests,
			"Core test blocks admin escalation through invites",
			"admin_can_only_invite_member_or_viewer",
		),
		textCheck(
			"security-admin-test",
			sources.corePolicyTests,
			"Core test limits security admin to audit/security views",
			"security_admin_can_view_audit_but_cannot_invite_members",
		),
		textCheck(
			"billing-admin-test",
			sources.corePolicyTests,
			"Core test limits billing admin away from audit",
			"billing_admin_can_manage_billing_but_cannot_view_audit",
		),
		textCheck(
			"viewer-readonly-test",
			sources.corePolicyTests,
			"Core test keeps viewer read-only",
			"viewer_is_read_only",
		),
		textCheck(
			"service-policy-call",
			sources.authzService,
			"Identity Service authorization calls the shared role policy",
			"is_allowed(access.role, action, effective_resource)",
		),
		textCheck(
			"denied-audit-call",
			sources.authzService,
			"Denied workspace actions are audited before returning an error",
			"record_permission_denied(db, &access, action, effective_resource, headers).await?",
		),
		textCheck(
			"audit-insert",
			sources.authzDb,
			"Permission denied decisions insert an audit event",
			"record_event_tx",
		),
		textCheck(
			"audit-action",
			sources.authzDb,
			"Permission denied audit uses stable action literal",
			'action: "permission.denied"',
		),
		textCheck(
			"audit-metadata-helper",
			sources.authzDb,
			"Permission denied audit metadata is generated through a tested helper",
			"permission_denied_metadata",
		),
		textCheck(
			"audit-metadata-test",
			sources.authzDb,
			"Permission denied audit metadata has a unit test",
			"permission_denied_metadata_captures_role_action_and_target_role",
		),
		textCheck(
			"decision-route",
			sources.authzRoutes,
			"Authz decision endpoint is routed",
			"/authz/decision",
		),
		textCheck(
			"decision-openapi",
			sources.openapi,
			"Authz decision endpoint is present in OpenAPI",
			'"/authz/decision"',
		),
	];
}
...
function validateReport(report) {
	...
	for (const check of report.checks) {
		...
		if (!Object.values(sources).includes(check.path))
			errors.push(
				`${check.id}: path is not in workspace membership source contract`,
			);
	...
	if (!sameItems(report.generation?.sources, Object.values(sources))) {
		errors.push(
			`${outputPath}: generation.sources must match workspace membership source contract`,
		);
	}
	if (
		!sameItems(report.generation?.targeted_tests, [
			...
			"cargo test -p nvbes-identity-service permission_denied_metadata_captures_role_action_and_target_role --locked",
		])
	) {
	...
	for (const command of report.generation?.targeted_tests ?? []) {
		errors.push(
			...validateProofCommand(
				{ id: "workspace-membership-roles", proof: command },
				packageScripts,
			),
		);
	}
...
if (write) {
	...
	console.log(
		`Workspace membership roles evidence written to ${markdownPath} and ${outputPath}`,
	);
	process.exit(0);
}
...
for (const [path, expected] of [
	...
	if (!existsSync(path))
		errors.push(
			`${path}: missing; run node tools/migration/workspace-membership-roles.mjs --write`,
		);
	else if (readFileSync(path, "utf8") !== expected)
		errors.push(
			`${path}: stale; run node tools/migration/workspace-membership-roles.mjs --write`,
		);
}
...
console.log(
	`Workspace membership roles evidence: ok (${summary.passed}/${summary.checks} checks passed)`,
);
