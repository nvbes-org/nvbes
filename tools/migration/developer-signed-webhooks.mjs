...
function buildChecks() {
	return [
		openApiMethodCheck("openapi-webhooks-list", "/developer/webhooks", "get", "OpenAPI exposes developer webhook listing"),
		openApiMethodCheck("openapi-webhooks-create", "/developer/webhooks", "post", "OpenAPI exposes developer webhook creation"),
		openApiMethodCheck("openapi-webhooks-delete", "/developer/webhooks/{endpointId}", "delete", "OpenAPI exposes developer webhook revocation"),
		textCheck("openapi-export-webhooks", sources.openapiExport, "OpenAPI export includes developer webhook handlers", "crate::domains::developer::webhooks_routes::create_webhook"),
		textCheck("public-route-secret", sources.publicRoutes, "Webhook creation returns signing secret once", "CreateDeveloperWebhookEndpointResponse"),
		textCheck("public-route-manage-permission", sources.publicRoutes, "Webhook mutations require manage permission", "DeveloperPermission::WebhooksManage"),
		textCheck("console-replay-route", sources.consoleRoutes, "Developer console exposes delivery replay route", "/developer/console/webhooks/deliveries/{deliveryId}/replay"),
		textCheck("delivery-hmac", sources.delivery, "Webhook signatures use HMAC-SHA256", "type HmacSha256 = Hmac<Sha256>"),
		textCheck("delivery-header", sources.delivery, "Signature header includes timestamp and v1 digest", 'format!("t={timestamp},{WEBHOOK_SIGNATURE_VERSION}={signature}")'),
		textCheck("delivery-event-binding", sources.delivery, "Signature binds event id into signed payload", "mac.update(event_id.to_string().as_bytes())"),
		textCheck("delivery-payload-binding", sources.delivery, "Signature binds raw payload bytes", "mac.update(payload)"),
		textCheck("delivery-window", sources.delivery, "Signature freshness window rejects replay outside tolerance", "WEBHOOK_SIGNATURE_TOLERANCE_SECONDS"),
		textCheck("delivery-replay-classifier", sources.delivery, "Replay classifier only allows failed or pending deliveries", '"failed" | "pending" => WebhookReplayDecision::Replayable'),
...
function validateReport(report) {
	...
	for (const check of report.checks) {
		...
		if (!Object.values(sources).includes(check.path)) errors.push(`${check.id}: path is not in developer signed webhook source contract`);
	...
	for (const command of report.generation?.targeted_tests ?? []) {
		errors.push(...validateProofCommand({ id: "developer-signed-webhooks", proof: command }, packageScripts));
	}
}
...
console.log(`Developer signed webhook evidence: ok (${summary.passed}/${summary.checks} checks passed)`);
