...
const sources = {
	...
	loginPwdRoute:
		"apps/identity-service/src/identity.domains.auth.routes.login.pwd.rs",
	loginIdentifierRoute:
		"apps/identity-service/src/identity.domains.auth.routes.login.identifier.rs",
	sessionMgmt:
		"apps/identity-service/src/identity.domains.auth.sessions.mgmt.rs",
	sessionMgmtTests:
		"apps/identity-service/src/identity.domains.auth.sessions.mgmt.tests.rs",
	sessionRefresh:
		"apps/identity-service/src/identity.http.middleware.jwt.session_refresh.rs",
	openapi: "apps/identity-service/openapi.json",
...
function buildChecks() {
	return [
		textCheck(
			"identifier-route",
			sources.loginIdentifierRoute,
			"Login identifier route is present",
			"challenge_identifier",
		),
		textCheck(
			"password-route",
			sources.loginPwdRoute,
			"Password login route is present",
			"challenge_pwd",
		),
		textCheck(
			"login-throttle",
			sources.loginVerify,
			"Primary login applies throttle rules",
			"LOGIN_THROTTLE_ACTION",
		),
		textCheck(
			"password-check",
			sources.loginVerify,
			"Primary login verifies stored password and upgrades weak hashes",
			"password::verify_and_check_rehash(",
		),
		textCheck(
			"login-failed-risk",
			sources.loginVerify,
			"Failed login records risk event",
			'"login_failed"',
		),
		textCheck(
			"login-failed-audit",
			sources.loginVerify,
			"Failed login records audit event",
			'"auth.login_failed"',
		),
		textCheck(
			"session-token",
			sources.loginSession,
			"Successful login issues an opaque session-bound browser token",
			"token::issue(session_id)",
		),
		textCheck(
			"session-cache",
			sources.loginSession,
			"Successful login stores cached session",
			"set_session(redis, &cached_session",
		),
		textCheck(
			"login-success-risk",
			sources.loginSession,
			"Successful login records risk event",
			'"login_success"',
		),
		textCheck(
			"login-success-audit",
			sources.loginSession,
			"Successful login records audit event",
			'"auth.login_success"',
		),
		textCheck(
			"logout-route",
			sources.openapi,
			"OpenAPI contains logout route",
			'"/auth/logout"',
		),
		textCheck(
			"sessions-route",
			sources.openapi,
			"OpenAPI contains session listing route",
			'"/auth/sessions"',
		),
		textCheck(
			"logout-state-helper",
			sources.sessionMgmt,
			"Logout revocation state is isolated from audit persistence",
			"revoke_logout_session_state",
		),
		textCheck(
		...
		textCheck(
			...
			sources.sessionMgmt,
			"Logout deletes the current session cache",
			"delete_session(redis, &user_id.to_string(), &session_id.to_string())",
		),
		textCheck(
			"logout-audit",
			sources.sessionMgmt,
			"Logout records session revoked audit event",
			'"auth.session_revoked"',
		),
		textCheck(
			"logout-test",
			sources.sessionMgmtTests,
			"Logout state test proves only current session and refresh token are revoked",
			"logout_state_revokes_current_session_and_refresh_token_only",
		),
		textCheck(
			"bearer-authentication",
			sources.sessionRefresh,
			"Session middleware authenticates OAuth bearer requests",
			"authorization_bearer_token",
		),
		textCheck(
			"browser-authentication",
			sources.sessionRefresh,
			"Session middleware authenticates authuser-scoped browser cookies",
			"browser_session_token_with_authuser",
		),
		textCheck(
			"browser-session-validation",
			sources.sessionRefresh,
			"Browser cookies are validated against central session state",
			"authenticate_browser_session",
		),
		textCheck(
			"reauthentication-recovery",
			sources.sessionRefresh,
			"Terminal authentication failures require explicit reauthentication",
			"requiring_reauthentication",
		),
		textCheck(
			"reauthentication-test",
			sources.sessionRefresh,
			"Session recovery semantics are covered by a unit test",
			"terminal_session_authentication_errors_require_reauthentication",
		),
		textCheck(
			"session-migration-decision",
			sources.riskRegister,
			"Session migration risk has explicit clearing/login return decision",
			"explicit logout/session clearing and a validated login return flow",
		),
	];
}
...
function validateReport(report) {
	...
	for (const check of report.checks) {
		...
		if (!Object.values(sources).includes(check.path))
			errors.push(
				`${check.id}: path is not in identity login/session source contract`,
			);
	...
	if (!sameItems(report.generation?.sources, Object.values(sources))) {
		errors.push(
			`${outputPath}: generation.sources must match identity login/session source contract`,
		);
	}
	if (
		!sameItems(report.generation?.targeted_tests, [
			"cargo test -p nvbes-identity-service terminal_session_authentication_errors_require_reauthentication --locked",
			"cargo test -p nvbes-identity-service logout_state_revokes_current_session_and_refresh_token_only --locked",
	...
	for (const command of report.generation?.targeted_tests ?? []) {
		errors.push(
			...validateProofCommand(
				{ id: "identity-login-session", proof: command },
				packageScripts,
			),
		);
	}
...
function serializeMarkdown(data) {
	...
	lines.push(
		...
		data.summary.failed === 0
			...
			: "Identity login/logout/session authentication parity evidence is blocked until failed checks pass.",
		"",
		"## Regeneration",
		"",
		"```bash",
...
if (write) {
	...
	console.log(
		`Identity login session evidence written to ${markdownPath} and ${outputPath}`,
	);
	process.exit(0);
}
...
for (const [path, expected] of [
	...
	if (!existsSync(path))
		errors.push(
			`${path}: missing; run node tools/migration/identity-login-session.mjs --write`,
		);
	else if (readFileSync(path, "utf8") !== expected)
		errors.push(
			`${path}: stale; run node tools/migration/identity-login-session.mjs --write`,
		);
}
...
console.log(
	`Identity login session evidence: ok (${summary.passed}/${summary.checks} checks passed)`,
);
