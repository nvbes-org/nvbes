...
function buildChecks() {
	return [
		textCheck("provider-public-codes", sources.provider, "Billing provider model exposes Stripe, Mollie and CB", 'PROVIDER_CODES: &[&str] = &["stripe", "mollie", "cb"]'),
		textCheck("provider-code-test", sources.providerTests, "Provider code tests cover CB", 'provider_code("cb")'),
		textCheck("cb-services", sources.cbProvider, "CB adapter models Safe'R, Updat'R and Fast'R services", '["safe_r", "updat_r", "fast_r"]'),
		textCheck("cb-safe-r", sources.cbProvider, "CB Safe'R excludes recurring payments", "safe_r_excludes_recurring"),
		textCheck("cb-updat-r", sources.cbProvider, "CB Updat'R requires stored credential context", "stored_credential_required"),
		...
		textCheck("provider-subscription-primary-not-fallback", sources.providerSubscriptions, "Primary provider subscriptions are not fallback candidates", "primary_provider_subscription_is_never_marked_as_fallback"),
		textCheck("provider-subscription-inactive-not-fallback", sources.providerSubscriptions, "Inactive provider subscriptions are not fallback candidates", "inactive_non_primary_provider_subscription_is_not_fallback_eligible"),
		textCheck("workspace-effects-primary-only", sources.workspaceEffects, "Webhook workspace effects apply only to primary provider subscription", "workspace_effects_apply_only_to_primary_provider_subscription"),
		textCheck("portal-provider-list", sources.portalSubscriptions, "Portal subscriptions expose provider-neutral primary and fallback state", "fallback_eligible"),
		textCheck("cb-migration", sources.cbMigration, "Billing database enum supports CB provider", "ADD VALUE IF NOT EXISTS 'cb'"),
		textCheck("billing-client-cb", sources.billingClientProvider, "Billing TypeScript client supports CB provider", "['stripe', 'mollie', 'cb']"),
		textCheck("graphql-cb", sources.graphqlSchema, "GraphQL Billing provider enum supports CB", "CB"),
	];
}
...
function validateReport(report) {
	...
	for (const check of report.checks) {
		...
		if (!Object.values(sources).includes(check.path)) errors.push(`${check.id}: path is not in billing multi-PSP source contract`);
	...
	for (const command of report.generation?.targeted_tests ?? []) {
		errors.push(...validateProofCommand({ id: "billing-multi-psp-continuity", proof: command }, packageScripts));
	}
}
...
console.log(`Billing multi-PSP continuity evidence: ok (${summary.passed}/${summary.checks} checks passed)`);
