...
export function setup() {
  ...
  if (response.status !== 200) {
    throw new Error(`Account service readiness failed with status ${response.status}`);
  }
}
...
function health(target) {
  group('health', () => {
    ...
    check(response, {
      ...
      'health exposes request id': (result) => Boolean(result.headers['X-Request-Id']),
    });
  });
}
...
function openidConfiguration(target) {
  group('openid configuration', () => {
    ...
    check(response, {
      ...
      'openid configuration has issuer': (result) => Boolean(json(result)?.issuer),
      'openid configuration has token endpoint': (result) => Boolean(json(result)?.token_endpoint),
    });
  });
}
