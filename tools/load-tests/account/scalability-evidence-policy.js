...
export function validateScalabilityComparisonSchema(comparison) {
  ...
  assert(comparison.runs.length === 4, 'scalability requires exactly 1, 2, 4 and 8 replicas');
  ...
  for (const run of comparison.runs) {
    ...
    assert(run.metadata !== run.summary, 'metadata and summary paths must be distinct');
  }
}
...
export function validateScalabilityMetadata(metadata, expectedSummaryFile) {
  ...
  assert(metadata.schemaVersion === 1, 'every metadata file must use schemaVersion 1');
  assert(metadata.profile === 'scalability', 'every run must use the scalability profile');
  assert(
    ['account-api', 'account-session'].includes(metadata.suite),
    'every run must use an Account load suite',
  );
  assert(metadata.targetEnvironment === 'staging', 'scalability evidence must come from staging');
  assert(
    metadata.configuredDuration === '15m',
    'scalability configuredDuration must be exactly 15m',
  );
  assert(metadata.configuredIterations === null, 'scalability configuredIterations must be null');
  ...
  assert(
    typeof metadata.release === 'string' && /^[a-f0-9]{7,64}$/u.test(metadata.release),
  ...
  assert(metadata.summaryFile === expectedSummaryFile, 'metadata and summary pair mismatch');
}
