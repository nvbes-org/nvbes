...
const baseUrl = normalizedTarget(
  __ENV.IDENTITY_SERVICE_BASE_URL || 'http://host.docker.internal:4001',
);
...
export function setup() {
  rejectLoadTarget(
    ...
    __ENV.IDENTITY_LOAD_ALLOWED_ORIGINS,
    __ENV.IDENTITY_PRODUCTION_DENIED_ORIGINS,
  );
  ...
  if (response.status !== 200) {
    throw new Error(`Identity service readiness failed with status ${response.status}`);
  }
}

export default function identityPublicJourney() {
  ...
  sleep(thinkTimeSeconds(__ENV.IDENTITY_K6_THINK_TIME_SECONDS, 0.2));
}
  ...
  group('health live', () => {
    ...
    check(response, {
      ...
      'health/live identifies service': (result) =>
        json(result)?.service === 'nvbes-identity-service',
    });
  });
}
