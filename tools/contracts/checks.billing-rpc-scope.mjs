const runtimeImplementedBillingRpcs = [
  {
    proto: 'GetBillingOverview',
    method: 'get_billing_overview',
    evidence: 'fetch_workspace_billing_overview',
  },
  { proto: 'GetBillingPortal', method: 'get_billing_portal', evidence: 'fetch_portal_view' },
  {
    proto: 'CreateCheckout',
    method: 'create_checkout',
    evidence: 'create_billing_checkout_session',
  },
  { proto: 'CreatePortal', method: 'create_portal', evidence: 'create_billing_portal_session' },
  { proto: 'GetEntitlements', method: 'get_entitlements', evidence: 'entitlement_snapshot' },
  { proto: 'ListInvoices', method: 'list_invoices', evidence: 'fetch_portal_invoices' },
  { proto: 'GetInvoice', method: 'get_invoice', evidence: 'fetch_portal_invoices' },
  { proto: 'IngestUsage', method: 'ingest_usage', evidence: 'ingest_usage_event' },
  {
    proto: 'RunReconciliation',
    method: 'run_reconciliation',
    evidence: 'run_ledger_reconciliation',
  },
];
...
const futureDeclaredBillingRpcs = [
  ...
  {
    proto: 'GetAdminCommandCenterBillingMetrics',
    method: 'get_admin_command_center_billing_metrics',
  },
...
export function checkBillingRpcRuntimeScope({ errors, gatewaySource, proto, protoPath, service }) {
  if (!service.includes('impl BillingService for BillingGrpcService')) {
    errors.push(
      'apps/billing-service/src/billing.grpc.service.rs: missing impl BillingService for BillingGrpcService',
    );
  }
  ...
  for (const rpc of runtimeImplementedBillingRpcs) {
    ...
    if (!service.includes(`async fn ${rpc.method}`) || !service.includes(rpc.evidence)) {
      errors.push(
        `apps/billing-service/src/billing.grpc.service.rs: missing runtime Billing RPC ${rpc.method}`,
      );
    }
  }
  ...
  for (const rpc of futureDeclaredBillingRpcs) {
    ...
    if (gatewaySource.includes(`.${rpc.method}(`)) {
      errors.push(
        `apps/gateway-cloud: must not call future-declared Billing RPC ${rpc.method} before runtime implementation exists`,
      );
    }
  }
}
