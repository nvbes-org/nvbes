export function checkOpenApiTaxonomy({ errors, manifest, readJson }) {
	...
	if (accountApi?.surface !== "account" || accountApi?.document !== "apps/account-service/openapi.json") {
		errors.push("contracts/openapi/manifest.json: account-service must describe the account surface");
	}
	...
	if (
		...
		errors.push("contracts/openapi/manifest.json: cloud-public-v1 must describe the Cloud public /v1 surface");
	}
